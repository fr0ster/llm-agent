...
This is a **storage-level** guarantee — it improves recall (both records survive). Tool *selection* is a separate layer: `resolve()` deduplicates tools by name because LLM tool-calling requires unique names, and the executor resolves a call via `toolClientMap.get(name)`. So when two servers expose the same tool name, the call still lands on the first client. Routing a specific colliding tool to its server needs tool-name namespacing across exposure, calling, and the executor — a distinct change, tracked separately.
