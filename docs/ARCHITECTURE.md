...
| `IToolNamespace` | Renames a colliding MCP tool's exposed (LLM/RAG-visible) name; bare when unique | `defaultToolNamespace` (`${prefix}__${toolName}` on collision, prefix = `mcp[].name` else `s${slotIndex}`); inject custom via `SmartAgentBuilder.withToolNamespace` |
