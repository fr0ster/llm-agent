...
Each tool's RAG record id comes from an `IToolRecordKey` strategy. The default keeps `tool:${name}` for a single MCP server and disambiguates by client index once several are connected (the `mcp:` config accepts an array), so identically named tools from different servers no longer overwrite each other **in the store**. A consumer that knows its servers injects its own via `SmartAgentBuilder.withToolRecordKey` — keying by real server name, a per-server collection, or any scheme whose id keeps the `tool:` prefix (retrieval uses it to separate tools from skills; a key that drops it fails the boot). Server↔client is 1:1, so the client index identifies the server for the default.

This is a **storage-level** guarantee — it improves recall (both records survive). Tool *selection* used to be a separate, unresolved layer: `resolve()` deduplicated tools by name because LLM tool-calling requires unique names, and the executor resolved a call via `toolClientMap.get(name)`, so when two servers exposed the same tool name the call always landed on the first client — the second server's tool was unreachable. This is now closed by the `IToolNamespace` strategy (below): a colliding exposed name is renamed (`${prefix}__${toolName}`), and the owning client is wrapped so the executor's call is unwrapped back to the tool's original bare name on the wire — see [docs/INTEGRATION.md#itoolnamespace](INTEGRATION.md#itoolnamespace) for the full interface.
...
- **Colliding tool names across servers** — `buildNamespacedTools` (used by the internal tool registry, startup vectorization, and the pipeline's tool-select/tool-loop handlers) detects when two or more currently-active MCP clients expose the same tool name and renames only the *colliding* ones via an injectable `IToolNamespace` strategy (default `defaultToolNamespace`: bare when unique, `${prefix}__${toolName}` on a collision). `prefix` is the server's config `name` (`mcp[].name`, a stable human-readable label validated against `^[a-zA-Z0-9_-]+$` and required to be unique across configured servers) or else `s${slotIndex}`. The owning client is wrapped (`bindToolCallName`) so every executor's `client.callTool(exposedName, ...)` is transparently unwrapped back to the tool's ORIGINAL bare name on the wire — no executor call site needs to know namespacing happened. **UX note:** the model only ever sees a namespaced name (e.g. `s0__Search` or, with `mcp[].name: weather`, `weather__Search`) on a genuine collision; a uniquely-named tool is always exposed bare, unaffected by this mechanism. Inject a custom strategy via `SmartAgentBuilder.withToolNamespace()`; see [docs/INTEGRATION.md#itoolnamespace](INTEGRATION.md#itoolnamespace).
- **Colliding tool names on the SERVER pipelines (`llm-agent-server-libs`, #244 addendum)** — the bullet above covers `llm-agent-libs`'s own internal `McpToolRegistry` (`flat`/`dag`); the `controller`, `linear`, and `stepper` server pipelines route through separate seams (`SmartServer.buildMcpBridge`/`callMcp`, the controller's own bridge, `makeToolsRagHandle`) that used to compare tool names *bare*. `SmartServer` now computes ONE authoritative namespaced snapshot (`{ tools, provenance }`) — either harvested from the startup builder when it owns the MCP connection itself (the yaml-only path), or built once server-side (`buildNamespacedTools` over `_sharedMcpClients`) when a consumer/seam supplies ready clients — and every seam *rebinds* that same snapshot's `provenance` (`exposedName → {slotIndex, originalName}`) onto its OWN client set via one shared `buildNamespacedMcpBridge(toolClientMap, classifier)`, instead of re-deriving names per seam. This is an **accepted frozen-snapshot tradeoff**: the snapshot is built once at boot/infra-build, so a tool that only appears after a post-boot MCP reconnect is not routable on these seams until the process restarts (`flat`/`dag` are unaffected — their internal registry still refreshes on `toolsChanged`). Per-seam client targets are UNCHANGED (isolation-preserving, #213): the controller still dispatches to the SESSION's own `scope.parts.mcpClients` (rebound by `slotIndex`, not array index, via `rebindProvenanceToClients`), `SmartServer.callMcp` (the `linear`/`stepper` `ctx.callMcp` bridge) still dispatches to the GLOBAL `_sharedMcpClients`. Fail-loud is preserved byte-for-byte: the shared bridge keeps the same `IMcpFailureClassifier` classify/throw-vs-`isError` split `buildMcpBridge` always had. A server with no writable tools-RAG store still gets a namespaced, selectable, callable catalog (the snapshot build never depends on RAG write capability); a stale/foreign RAG record (no matching catalog entry) is skipped, never surfaced. Two additive `BuildAgentDeps` DI seams: `connectMcpWithDescriptors` (a `connectMcp` sibling that also reports per-slot `{slotIndex, label}` descriptors) and `toolNamespace` (the same `IToolNamespace` instance reaching both the startup builder's snapshot and the server's own fallback build). See [docs/INTEGRATION.md#itoolnamespace](INTEGRATION.md#itoolnamespace) for the DI seams and [docs/TROUBLESHOOTING.md](TROUBLESHOOTING.md) for the symptom this fixes.
...
| `IToolNamespace` | Renames a colliding MCP tool's exposed (LLM/RAG-visible) name; bare when unique | `defaultToolNamespace` (`${prefix}__${toolName}` on collision, prefix = `mcp[].name` else `s${slotIndex}`); inject custom via `SmartAgentBuilder.withToolNamespace` (library) or `BuildAgentDeps.toolNamespace` (server — reaches both the yaml-builder snapshot and the server's own fallback build) |
