...
**Why a bare `connectMcp` can't get `mcp[].name` labels automatically:** a bare `connectMcp` returns an opaque `IMcpClient[]` with no guaranteed correspondence to the `mcp[]` config entries (a custom connector may connect a different set, order, or count), so a label cannot be attributed to a client by array index without risking mislabeling the wrong server. The `connectMcpWithDescriptors` seam exists precisely so the consumer — who knows their own client↔config mapping — reports `{ slotIndex, label }` themselves; a bare `connectMcp` falls back to slot-index prefixes (`s0__`/`s1__`), which are always safe.
