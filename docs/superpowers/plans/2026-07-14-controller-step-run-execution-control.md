...
- [ ] **Step 3: Implement:**
  ...
  - **Before each `executor.send` (top of round):** `const r = budget.shouldContinueRound(state()); if (!r.continue) { …writeControlFailure(r.reason) + settle('failed') + awaiting-replan… return settle('failed'); }`.
  - **executor.send with signal + abort mapping:** `let res; try { res = await deps.executor.send(messages, offeredTools, { ...ctx.options, signal: budget.signal }); } catch (e) { if (budget.signal.aborted) { …writeControlFailure('step-timeout') + settle('failed')… return settle('failed'); } throw e; }` — then AFTER the return, `if (budget.signal.aborted) { …control-failure('step-timeout')… return settle('failed'); }` BEFORE the existing `res.kind==='error'` retry branch.
  - **Replace the external cap (@1283)** `if (inFlight && inFlight.toolCallCount + 1 > maxToolCalls)` with `const g = budget.canExecuteTool(state()); if (!g.continue) { …writeControlFailure(g.reason)… return settle('failed'); }` — consulted BEFORE the increment.
  - **Replace the internal cap (@1342-1348)** post-increment check: move to `budget.canExecuteTool(state())` BEFORE the `inFlight.toolCallCount += 1` increment; on `!continue` → control-failure(reason). (So the increment happens only after `canExecuteTool` allows the call — matching the spec's before-increment model.)
  - **callMcp with signal:** `result = await deps.callMcp(name, args, budget.signal);` — the existing catch already handles `McpError` (20.4.0). ADD, at the TOP of that catch: `if (budget.signal.aborted) { …writeControlFailure('step-timeout') + settle('failed')… return settle('failed'); }` BEFORE the `McpError instanceof` escalate — so a step-timeout cancellation is NOT treated as MCP-unavailable.
...
- Modify: `packages/llm-agent-libs/src/builder.ts` (`withStepExecutionControl`/`withRunExecutionControl`)
...
- Produces: `ctx.stepExecutionControl`/`ctx.runExecutionControl` populated from DI; controller uses `ctx.stepExecutionControl ?? new DefaultStepExecutionControl()`, `ctx.runExecutionControl ?? new NoopRunExecutionControl()`.

> Mirror the existing `toolLoopContextStrategyFactory` DI threading (same sites) — but these are shared INSTANCES (stateless controls), not factories. Optional/additive everywhere.

- [ ] **Step 1: Failing tests:**
  - (a) `builder.withStepExecutionControl(custom)` → resolved ctx carries `custom`; `withRunExecutionControl(custom)` similarly.
  - (b) Controller composition (mirror `controller-context-wiring.test.ts`): no injection → the handler receives a `DefaultStepExecutionControl` / `NoopRunExecutionControl`; injection → the consumer's instances. And `callMcp` passed to the handler forwards the signal to `buildMcpBridge`.

- [ ] **Step 2: Run → FAIL.**

- [ ] **Step 3: Implement:**
  - Add `stepExecutionControl?: IStepExecutionControl` + `runExecutionControl?: IRunExecutionControl` to `IPipelineContext` (pipeline-plugin.ts) and `BuildAgentDeps` (smart-server.ts). `smart-server` populates `ctx.stepExecutionControl`/`ctx.runExecutionControl` from `BuildAgentDeps` (undefined when not injected).
  - `builder.ts`: `private _stepExecutionControl?` + `withStepExecutionControl(c): this`; `private _runExecutionControl?` + `withRunExecutionControl(c): this` (mirror `withMcpFailureClassifier`); thread into `BuildAgentDeps`/deps in `build()`.
...
- Grep the existing `withMcpFailureClassifier` / `toolLoopContextStrategyFactory` DI threading as the exact template for Task 8 (same sites: pipeline-plugin.ts, builder.ts, smart-server.ts, controller-factory.ts).
