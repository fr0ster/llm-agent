...
// ragStores.tools MUST exist (query can return []) — it is the gate at tool-loop.ts:221
//   that lets execution reach the read-only restore branch (262); without it the guard is inert.
