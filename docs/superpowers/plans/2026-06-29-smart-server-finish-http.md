...
- **Each PR = a complete concern: after Task 2 no inline body remains for the 7 TARGETED routes** (sessions list/resume/delete, models, embedding-models, usage, health) — each `handle:` is a one-line call to an `http/` free function. The 3 already-delegating routes (config, messages, chat) are **out of scope by design** and stay as scoped inline delegators: messages/chat already one-line into `_withSession`/`handleChat`/`handleAdapterRequest`; config keeps its small inline GET/PUT/405 method-dispatch that already delegates PUT to `handleConfigUpdate` (extracted in a prior PR). **After Task 3 the bespoke `IToolsRagHandle` is gone** — `buildToolsRagHandle` is a one-line delegation to `makeToolsRagHandle(...)`.
...
- [ ] **Rewire `_buildRouteTable`** — replace the 3 sessions `handle:` bodies with one-line calls (registration arrow stays inside the class, so it may read the privates):
  ...
  ```
  **REMOVE `handleListSessions`/`handleResumeSession`/`handleDeleteSession` from the `import { … } from './session-lifecycle/index.js'` block (~469-471).** After Task 1 moves the sessions route bodies out, these three are no longer USED inside `smart-server.ts` (their only callers — lines ~2534/2568/2612 — leave the file), so with `noUnusedLocals: true` (tsconfig.base.json:15) leaving them in the import block is a BUILD ERROR. The separate `export { … } from './session-lifecycle/index.js'` re-export block (~481-483) is INDEPENDENT of the import bindings — KEEP it unchanged so the package barrel still surfaces those symbols. (A `export { x } from './m.js'` re-export does not consume a local `import { x }` binding — they are two distinct statements.) Do NOT add the NEW `handleSessionsList`/`handleSessionResume`/`handleSessionDelete` functions to any barrel.
...
- [ ] **Rewire `_buildRouteTable`** — replace the 4 `handle:` bodies with one-liners (order/`method`/`match` unchanged):
  ...
  ```
- [ ] **Verify the complete-concern requirement:** read every `table.add(...)` in `_buildRouteTable` and confirm the **7 TARGETED routes** (sessions ×3, models, embedding-models, usage, health) are now one-line calls to `http/` free functions — no inline body for any of the 7. The 3 out-of-scope routes stay as scoped delegators by design: config keeps its small inline GET/PUT/405 dispatch (already delegates PUT → `handleConfigUpdate`), messages → `_withSession`/`handleAdapterRequest`, chat → `_withSession`/`handleChat`. Do NOT touch those 3.
- [ ] **Post-check file sizes:** `wc -l packages/llm-agent-server-libs/src/smart-agent/smart-server.ts` (expect well below 2775, trending toward ~2300 after Task 3) and `wc -l packages/llm-agent-server-libs/src/smart-agent/http/*.ts` — confirm NO newly-created `http/*.ts` module exceeds ~500 lines (they should each be well under; if one approaches it, that is a signal it absorbed too much and should be split).
...
- **Complete-concern check encoded:** Task 2 has an explicit step to read every `table.add` and confirm the 7 TARGETED routes are one-line calls (config/messages/chat stay scoped delegators by design); Task 3 reduces `buildToolsRagHandle` to a one-line delegation. ✔
- **`noUnusedLocals` (P1 review fix):** Task 1 explicitly REMOVES `handleListSessions`/`handleResumeSession`/`handleDeleteSession` from the `import` block (their in-file callers move to `sessions-route-handler.ts`) while KEEPING the independent `export { … } from './session-lifecycle/index.js'` re-export. ✔
