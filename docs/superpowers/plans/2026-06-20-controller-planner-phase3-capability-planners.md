...
| `src/smart-agent/controller/controller-coordinator-handler.ts` | Call `makeControllerPlanner(deps.plannerKind ?? 'smart-executor', ...)` (drop `deps.config.planner`); add a `deps.controllerPlanner?: IControllerPlanner` DI seam (Task 4 Step 2c — lets rewind tests inject a planner that emits `{kind:'rewind'}`, which the default smart/weak planners cannot). |
...
3. **Replan/error/partial/budget tests have MORE planner-client interactions** (a failed/partial step triggers a replan → another `callPlan` reply returning `{plan:[…remaining…]}` or `{plan:[]}`). Do NOT guess the exact reply count — RUN the test, read which call the scripted queue under-/over-feeds, and supply the matching plan-first reply. The handler/planner control flow is the spec; the test scripts adapt to it.
4. **Finalizer tests need BOTH a planner finalize reply AND `deps.finalizer`.** When the plan is exhausted, `SmartExecutorPlanner.stepAtCursor` ALWAYS makes a planner-client `FINALIZE_SYSTEM` call to return `{kind:'done', result}` — REGARDLESS of `deps.finalizer`. The handler then: if `deps.finalizer` is set → it calls `deps.finalizer.finalize(...)` and the COMPOSED answer is what's surfaced (the planner's done.result, passed as `legacyAnswer`, is IGNORED); if `deps.finalizer` is absent → the planner's done.result (`legacyAnswer`) is surfaced. So a finalizer test still needs a trailing plain-text planner finalize reply (to reach `done`) AND its `deps.finalizer` whose `finalize()` result is the asserted answer. (Handler refs: done→legacyAnswer at ~773/786; finalizer-vs-legacy at ~1443/1451/1487.)
5. Iterate until ALL non-rewind `ControllerCoordinatorHandler` tests pass; then handle the rewind tests (point 6).

**6. REWIND tests — re-script is IMPOSSIBLE; use a planner DI seam (explicit decision).** The handler's `next.kind === 'rewind'` branch (bump `rewindsUsed`, escalate at `maxRewinds`) is UNCHANGED and stays (out of §C scope). But `SmartExecutorPlanner`/`WeakExecutorPlanner` parse create/replan replies with `parsePlan` (only `{plan:[…]}`) and `stepAtCursor` only ever returns `{kind:'next'}`/`{kind:'done'}` — **neither default planner can emit `{kind:'rewind'}`** (a `{kind:'rewind'}` planner-client reply now hits `parsePlan` → `null` → a format-retry, NOT a rewind). The OLD rewind tests drove rewind through the now-deleted `IncrementalPlanner.parseNextStep` path, so they CANNOT be migrated by re-scripting the planner client. Decision (the reviewer's option (b), chosen to KEEP coverage of the still-live handler branch): test the handler's rewind branch directly via a **planner DI seam**.

   (a) Add the seam to the handler (small product-code change). In `ControllerHandlerDeps` add:
   ```ts
     /** Test/composition seam: a pre-built planner. When set, the handler uses it
      *  verbatim instead of `makeControllerPlanner(plannerKind, planner, …)`. Lets a
      *  consumer (or a test) supply an IControllerPlanner that emits behaviours the
      *  default smart/weak planners do not (e.g. `{kind:'rewind'}`). */
     controllerPlanner?: IControllerPlanner;
   ```
   (Import `IControllerPlanner` from `./types.js`.) At the handler's planner-construction site (Task 3 Step 1, where `makeControllerPlanner(deps.plannerKind ?? 'smart-executor', …)` is called), use the override when present:
   ```ts
   ...
   ```
   (b) Migrate the rewind tests (`rewind: one rewind then proceeds to done`, `rewind budget: escalates after exceeding maxRewinds`) to inject a fake `IControllerPlanner` via `deps.controllerPlanner` whose `next()` returns `{kind:'rewind', reason:'…'}` the scripted number of times, then `{kind:'next', step}` / `{kind:'done', result}` (and a no-op `commit?()`). This tests the handler's rewind handling in isolation — a better level than coupling to a planner's reply shape. Keep the assertions (`rewindsUsed`, the `maxRewinds` escalation) unchanged.

> Do NOT weaken or delete a test to make it pass. Each test asserts real handler behaviour (dispatch, tools, rewind, budgets, reviewer, finalizer, suspend/resume) UNCHANGED by Phase 3 — only HOW the test drives the planner changes (plan-first script for create/replan/finalize; a fake `IControllerPlanner` via the new seam for rewind). If a test's asserted handler behaviour genuinely cannot be reproduced, STOP and report it — that would be an unexpected behavioural regression, not a test-shape issue.
...
**P1 (round 6, external) — rewind tests** can't be re-scripted to plan-first (no default planner emits `{kind:'rewind'}`; the handler branch is unreachable via the planner client). Step 2c point 6 now adds a `deps.controllerPlanner?: IControllerPlanner` DI seam and migrates the rewind tests to inject a fake planner that emits `{kind:'rewind'}` — keeping the still-live handler rewind branch tested (decision: keep coverage via the seam, not obsolete-and-delete). **P2 (round 6) — finalizer tests** need BOTH a planner-client finalize reply (to reach `done`) AND `deps.finalizer` (whose composed result is surfaced; the planner's done.result is ignored when a finalizer is set) — Step 2c point 4 corrected (was "either/or").
