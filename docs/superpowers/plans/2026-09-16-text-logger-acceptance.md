...
- **Additive at runtime; the version is not this plan's to declare.** Nothing is removed, no existing path changes behaviour, and a consumer that keeps passing an `ILogger` sees exactly today's behaviour — no pre-existing test may need editing. But six readable option properties widen, and that breaks *source* compatibility for a consumer that reads one rather than setting it: `options.logger?.log(event)` on `ConnectionStrategyOptions`, `ComposeResilienceOptions`, `SessionGraphFactoryOptions`, `EmbedderResolutionOptions`, `RagResolutionOptions` or `SessionLifecycleOptions` compiled before and does not now. Measured: `error TS2339: Property 'log' does not exist on type 'AnyLogger'`. Widened *parameters* — `withLogger`, `FallbackLlmCallStrategy`'s constructor, `resolveAgentEmbedder`, `resolveToolsStoreEmbedder` — break nobody who calls them. Handing a logger in is unaffected; reading one needs a definedness check first, because these properties are optional and `normaliseLogger` takes a non-optional `AnyLogger`. What compiles, measured: `if (options.logger) normaliseLogger(options.logger).log(event);` This makes the release that carries this workstream a major (§10), decided at the release rather than here — merging changes no version.
...
- **The eight declarations above are the whole claim. What ACCEPTS them is not enumerated anywhere.** Anything that takes one of those types accepts either shape as well — `makeConnectionStrategy`, the `LazyConnectionStrategy` and `PeriodicConnectionStrategy` constructors, `SessionGraphFactory`, `composeResilientEmbedder`, `resolveEmbedder`, `makeRag`, `BuildAgentDeps.resolveEmbedder`, and anything you write yourself that accepts one. That set is deliberately not listed: it follows from the types rather than from a decision, it grows whenever a signature is added, and four hand-written versions of it here were each incomplete. Your compiler is the authority — ask it for references to the type, not this page. A textual search is not a substitute and must not be written into a document as one: `grep -rn AnyLogger packages` returns imports, re-exports, comments and generated `dist/*.d.ts` alongside the declarations, and it cannot see a site that takes `ConnectionStrategyOptions` without naming `AnyLogger` — which is exactly how `LazyConnectionStrategy` was missed twice.
