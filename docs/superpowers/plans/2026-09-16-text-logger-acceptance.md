...
- **Additive minor, with one qualification that belongs in the changelog.** Nothing is removed, no existing path changes behaviour, and a consumer that keeps passing an `ILogger` sees exactly today's behaviour — no pre-existing test may need editing. What is NOT unconditionally true is *source* compatibility for a consumer that **reads** a widened property rather than setting it: `options.logger?.log(event)` on `ConnectionStrategyOptions`, `ComposeResilienceOptions` or `SessionGraphFactoryOptions` compiled before and does not now. Measured: `error TS2339: Property 'log' does not exist on type 'AnyLogger'`. Handing a logger in is unaffected, and the consumer's fix is one line — `normaliseLogger(options.logger)`, or narrow with `isTextLogger`. State this in the changelog rather than claiming nothing can break.
...
- **Widening is inherited, and a list of declarations cannot contain what inherits it.** An interface that `extends` a widened one, or a constructor that takes it, accepts a text logger too — that is what `extends` means, not a decision a task makes. Here that is `MakeConnectionStrategyOptions` (it extends `ConnectionStrategyOptions`, so `makeConnectionStrategy` takes either shape) and `PeriodicConnectionStrategy`'s constructor, both in `llm-agent-mcp/src/strategies/`. Both reach `LazyConnectionStrategy`, which normalises before storing, so the safety property holds. Re-narrowing them to keep this list short would refuse a text logger at a place that handles it correctly, which is why the list is qualified instead. `docs/INTEGRATION.md` carries the full reachable set, and that is the consumer-facing answer.
