...
**Architecture:** `ITextLogger` is added as a re-export of `@mcp-abap-adt/interfaces-utils`' `ILogger` (`info`/`error`/`warn`/`debug`). Seams that *consume* a logger widen to `ILogger | ITextLogger` and normalise once, at the boundary, through one internal adapter that turns a `LogEvent` into a levelled text call. Two kinds of seam keep the event `ILogger` on purpose: those that *hand a logger out* (`IPipelineContext.logger`, `IPipelinePlugin`), and those that *forward one into* them (`PipelineDeps.logger`, `SmartAgentDeps.logger`) — widening either would put an `AnyLogger` in front of consumer plugins and break them, which is a major. The seams that widen by declaration are listed in Global Constraints, and what else accepts them is derived there rather than enumerated; "everywhere" would be false, and saying it would be worse than saying nothing.
...
- **Additive minor, with one qualification that belongs in the changelog.** Nothing is removed, no existing path changes behaviour, and a consumer that keeps passing an `ILogger` sees exactly today's behaviour — no pre-existing test may need editing. What is NOT unconditionally true is *source* compatibility for a consumer that **reads** a widened property rather than setting it: `options.logger?.log(event)` on `ConnectionStrategyOptions`, `ComposeResilienceOptions` or `SessionGraphFactoryOptions` compiled before and does not now. Measured: `error TS2339: Property 'log' does not exist on type 'AnyLogger'`. Handing a logger in is unaffected. Reading one needs a definedness check first: these properties are optional and `normaliseLogger` takes a non-optional `AnyLogger`, so `normaliseLogger(options.logger)` on its own does not compile either (`error TS2345: Argument of type 'AnyLogger | undefined' is not assignable to parameter of type 'AnyLogger'`). What compiles, measured: `if (options.logger) normaliseLogger(options.logger).log(event);` State this in the changelog rather than claiming nothing can break.
...
- **Input seams widen to `ILogger | ITextLogger`. This list is authoritative for what widens BY DECLARATION** — if a task, a docstring or the changelog disagrees about which declaration changed, this line wins. It does NOT say that nothing else accepts a text logger: whatever takes one of these types accepts one too, and the constraint after the list derives that set instead of naming it. "Seam" means a place a consumer hands llm-agent a logger for it to use; `normaliseLogger(logger)` and `isTextLogger(logger)` also accept `AnyLogger`, but they are the adapter itself rather than seams, so they are not on this list and do not belong on it:
...
- **What accepts a widened type is derived, never enumerated.** Two searches, two sets, and neither is a list to maintain by hand — three successive hand-written versions of it were each missing something. What widens by declaration: `grep -rn AnyLogger packages --include="*.ts"`, which returns the eight above. What *accepts* one of those types, and so takes either shape without deciding to: grep the interface name. On this branch that adds `LazyConnectionStrategy`'s constructor (`llm-agent-mcp/src/strategies/lazy-connection-strategy.ts:44`), `PeriodicConnectionStrategy`'s (`periodic-connection-strategy.ts:27`), `makeConnectionStrategy` through `MakeConnectionStrategyOptions extends ConnectionStrategyOptions` (`make-connection-strategy.ts:8`, `:23`), and `SessionGraphFactory`'s constructor (`llm-agent-libs/src/session/session-graph-factory.ts:160`, exported at `index.ts:211`). Every one of them reaches a normalising boundary before anything calls `.log`, which is why the documents are amended rather than the code re-narrowed.
...
Both logger shapes are accepted at the seams listed below, as those seams are declared. Anything that takes one of those option types accepts either shape as well — a property of the types, not a separate decision: today `LazyConnectionStrategy` and `PeriodicConnectionStrategy`'s constructors, `makeConnectionStrategy`, and `SessionGraphFactory`'s constructor. The inputs that deliberately stay event-only are at the end of this section.
