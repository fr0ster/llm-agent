...
- `packages/llm-agent/src/interfaces/pipeline-plugin.ts` — **(Task 5)** add `mcpFailureClassifier?: IMcpFailureClassifier` to `IPipelineContext` (the seam that reaches the controller path).
- `packages/llm-agent-server-libs/src/smart-agent/smart-server.ts` — **(Task 3)** `buildMcpBridge(clients, classifier)` classifies `listTools`/`callTool` errors (replacing hardcoded `isMcpUnavailable`); `this.callMcp` passes the instance classifier; **(Task 5)** populate `ctx.mcpFailureClassifier` from DI/config (default `DefaultMcpFailureClassifier`).
- `packages/llm-agent-server-libs/src/pipelines/controller.ts` + `packages/llm-agent-server-libs/src/factories/controller-factory.ts` — **(Task 3/5)** thread `ctx.mcpFailureClassifier` into `ControllerFactoryDeps` so the handler surfaces a thrown unavailable error loudly.
...
- [ ] **Implement:**
  - `buildMcpBridge(clients, classifier)`: replace the hardcoded `isMcpUnavailable(err)` checks (both the `listTools` and `callTool` error branches) with `await classifier.classify(err) === 'unavailable' ? throw err : return err.message`. `this.callMcp` (1790) passes the instance classifier (`this._mcpFailureClassifier ?? new DefaultMcpFailureClassifier()`).
  - Controller handler: ensure a thrown `McpError` from `deps.callMcp` at ~1285 is CAUGHT and surfaced loud (`abortTerminal` with `'MCP server unavailable: ' + err.message`), not swallowed. If the confirming test shows it already surfaces loud (propagates to a loud terminal), keep the guard test and make no handler change; if it degrades, add the catch+`abortTerminal`.
  - Thread the classifier from `ctx.mcpFailureClassifier` into `ControllerFactoryDeps` (controller.ts) only if the handler needs it directly (it may not — the bridge classifies; the handler only surfaces the throw). Prefer the minimal wiring the confirming test requires.
...
### Task 5 — DI threading: `IPipelineContext.mcpFailureClassifier` → controller + bridge + builder seam

The classifier must reach the CONTROLLER path (P1b), not only the generic builder. Primary seam = `IPipelineContext`; the builder setter is an additional programmatic convenience.

**Files:** modify `packages/llm-agent/src/interfaces/pipeline-plugin.ts` (`IPipelineContext.mcpFailureClassifier?`), `packages/llm-agent-server-libs/src/smart-agent/smart-server.ts` (store the instance classifier from DI/config, populate `ctx.mcpFailureClassifier`, pass it to `buildMcpBridge`), `packages/llm-agent-server-libs/src/pipelines/controller.ts` (read `ctx.mcpFailureClassifier` → deps if needed), `packages/llm-agent-libs/src/pipeline/handlers/*` (flat `classifyToolResult` reads `ctx.mcpFailureClassifier`), `packages/llm-agent-libs/src/builder.ts` (`withMcpFailureClassifier(...)` setter). Tests.

**Steps:**
- [ ] Add `mcpFailureClassifier?: IMcpFailureClassifier` to `IPipelineContext` (`pipeline-plugin.ts:37`); `IServerPipelineContext` inherits it. `npm run build`.
- [ ] **Failing test first:** (a) a builder test — `builder.withMcpFailureClassifier(custom)` → the resolved context/deps carry `custom`; default → `DefaultMcpFailureClassifier`. (b) a smart-server/context test — `ctx.mcpFailureClassifier` is populated (default) and reaches `buildMcpBridge` (assert the bridge built by `this.callMcp` uses it — reuse the Task 3 bridge seam). Mirror the `withMcpConnectionStrategy`/`withMcpRequestHeadersStrategy` threading tests.
- [ ] **Implement:** builder `private _mcpFailureClassifier?` + `withMcpFailureClassifier(c): this` (mirror @444/@453); smart-server stores the classifier (DI/config, default `DefaultMcpFailureClassifier`), populates `ctx.mcpFailureClassifier`, and passes it to `buildMcpBridge` (Task 3 already made the bridge accept it); the flat pipeline reads `ctx.mcpFailureClassifier` for `classifyToolResult` (Task 4 made it accept the param). Confirm the controller path (controller.ts → factory → handler/bridge) sees the same classifier instance.
- [ ] Run tests → GREEN. `npm run build` (all packages). SCOPED lint → commit: `feat(di): thread IMcpFailureClassifier through IPipelineContext + builder seam (reaches the controller path)`.
...
- `toMcpError`/`isMcpUnavailable` are shared helpers (lightweight, in llm-agent) — extending their recognition (Task 1) fixes the classification at the source for BOTH paths, and is "extend the component", not app glue.
