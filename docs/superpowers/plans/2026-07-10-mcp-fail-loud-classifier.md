...
**Files:** modify `packages/llm-agent-server-libs/src/smart-agent/smart-server.ts` (`buildMcpBridge` @565 + `this.callMcp` @1790) and `packages/llm-agent-server-libs/src/smart-agent/controller/controller-coordinator-handler.ts` (the `deps.callMcp` throw at ~1285). Tests: a `buildMcpBridge` unit test + the controller confirming test. **(Task 3 touches ONLY these two files — do NOT touch `controller-factory.ts` / `pipelines/controller.ts` here; threading the injected classifier from `ctx` is Task 5, which keeps Task 3 atomic and self-contained.)**
...
- [ ] **Failing test first — buildMcpBridge classifier:** `buildMcpBridge([client], customClassifier)` where `client.callTool` resolves `{ ok: false, error }` (the `IMcpClient` contract — a failed call returns a `Result`, it does not reject) with an error the CUSTOM classifier maps to `'unavailable'` → the bridge THROWS (not returns text); with the default classifier and a non-unavailable error → returns the text. (Assert the classifier is consulted, not hardcoded `isMcpUnavailable`.) **Probe test:** a custom classifier whose `classify(err, probeHealth)` returns `'unavailable'` ONLY when `probeHealth` is defined AND `await probeHealth() === false` — with a `client.healthCheck` stub resolving `{ ok: true, value: false }` (server down) the bridge THROWS; with the stub resolving `{ ok: true, value: true }` (server up) the bridge RETURNS the tool-error text. This proves `buildMcpBridge` actually hands the per-client health probe to the classifier (the interface seam is wired, not dead).
...
- [ ] **Implement:**
  - `buildMcpBridge(clients, classifier: IMcpFailureClassifier = new DefaultMcpFailureClassifier())` — the classifier is an OPTIONAL param with a default so existing callers/tests (`buildMcpBridge([client])`) compile UNCHANGED (backward-compatible; no call-site churn). Replace the hardcoded `isMcpUnavailable(err)` checks (both the `listTools` and `callTool` error branches) with `(await classifier.classify(err, probe)) === 'unavailable' ? throw err : return err.message` (the bridge is already async). **Pass the per-client health probe** so a health-confirming custom classifier can actually reach the server's `ping`: in each branch the relevant `client` is in scope, so build `const probe = client.healthCheck ? () => client.healthCheck!().then((r) => (r.ok ? r.value : false)) : undefined;` (guards the OPTIONAL `IMcpClient.healthCheck?`; unwraps `Result<boolean, McpError>` → `boolean`, treating a failed probe as `false` = down). The `DefaultMcpFailureClassifier` ignores `probe` (error-based), so default behavior is unchanged. **Leave the `this.callMcp` (1790) call site as `buildMcpBridge(clients)` (uses the default)** — passing a stored/injected classifier is Task 5 (do NOT reference `this._mcpFailureClassifier` here; it does not exist yet). This keeps Task 3 self-contained.
...
- [ ] **Change the signature:** `classifyToolResult(res: ToolRes, classifier: IMcpFailureClassifier = new DefaultMcpFailureClassifier(), probeHealth?: () => Promise<boolean>): Promise<ToolResultDecision>` — the unavailable decision now goes through `await classifier.classify(res.error, probeHealth)`; default classifier == today's `isMcpUnavailable` and ignores `probeHealth`, so behavior is UNCHANGED for existing callers. Update the `tool-loop-core.ts:349` call to `const decision = await classifyToolResult(res)` — **using the DEFAULT classifier only, no probe** (do NOT reference a threaded classifier or the client probe yet; both arrive in Task 5 via `IExecuteToolBatchArgs` + `toolClientMap`). This keeps Task 4 self-contained: it builds + commits green on its own (async + default), and Task 5 later passes the injected classifier as the 2nd arg and the per-client probe as the 3rd.
- [ ] **Failing test first:** with a custom classifier that returns `'unavailable'` for a `MCP_ERROR`, `await classifyToolResult(res, custom)` escalates; with the default, a `MCP_ERROR` does NOT escalate (unchanged); a `MCP_NOT_CONNECTED` escalates by default. **Probe passthrough:** a custom classifier that escalates only when `probeHealth` resolves `false` — `await classifyToolResult(res, custom, async () => false)` escalates, `await classifyToolResult(res, custom, async () => true)` does not — proving the 3rd arg reaches `classifier.classify`. Update ALL existing `classifyToolResult`/tool-loop callers + tests to `await`.
...
- [ ] Add `mcpFailureClassifier?: IMcpFailureClassifier` to: `IPipelineContext` (`pipeline-plugin.ts:37`, inherited by `IServerPipelineContext`), `IExecuteToolBatchArgs` (shared core), `SmartAgentDeps`, `PipelineDeps`, and `BuildAgentDeps` (the programmatic `buildAgent(deps)` seam — so an embedding consumer injects it). In `executeToolBatchWithHeartbeat`, update the Task-4 call site (`tool-loop-core.ts:349`, inside `for (const r of results)` where `const { tc, text, res } = r;` — `tc.name` is in scope). Derive the per-tool health probe from the SAME `toolClientMap` the core already destructures: `const client = toolClientMap.get(tc.name); const probe = client?.healthCheck ? () => client.healthCheck!().then((hr) => (hr.ok ? hr.value : false)) : undefined;` then `const decision = await classifyToolResult(res, args.mcpFailureClassifier, probe)` (classifier undefined → Task 4's default; probe lets a health-confirming classifier reach the server's `ping`). Both callers pass the classifier: `ToolLoopHandler` from `ctx.mcpFailureClassifier`; `SmartAgent._runStreamingToolLoop` from `this.deps.mcpFailureClassifier`.
