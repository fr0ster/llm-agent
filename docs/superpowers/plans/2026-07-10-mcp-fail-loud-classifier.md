...
- [ ] **Change the signature:** `classifyToolResult(res: ToolRes, classifier: IMcpFailureClassifier = new DefaultMcpFailureClassifier()): Promise<ToolResultDecision>` — the unavailable decision now goes through `await classifier.classify(res.error)`; default classifier == today's `isMcpUnavailable`, so behavior is UNCHANGED for existing callers. Update the `tool-loop-core.ts:349` call to `const decision = await classifyToolResult(res, classifier)` (the `classifier` threaded via `IExecuteToolBatchArgs` — Task 5).
