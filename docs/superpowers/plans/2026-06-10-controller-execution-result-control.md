...
// Exported: reused by the controller's relevantExtract fragment-selector (#1/plan-21).
...
export async function runScopedRecall(
   ...
   *  re-fetches (and toolCallCount resets per attempt), so the caller passes the
   *  GUARANTEED run bound `maxSteps × maxStepAttempts × maxToolCalls` (every distinct
   *  identityKey is seen before the cap). */
  kPrime: number,
