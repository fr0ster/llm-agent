...
- `DefaultStepExecutionControl` — `beginStep(ctx)` returns a budget where:
  - `signal`: use an **explicit `AbortController` + `setTimeout`/`clearTimeout`** (NOT `AbortSignal.timeout()`, which hands back no timer handle for `dispose()` to clear → orphan callbacks). When `ctx.budgets.perStepTimeoutMs` is set, `beginStep` starts `const t = setTimeout(() => ctrl.abort(new DOMException('step-timeout','TimeoutError')), perStepTimeoutMs)` and `dispose()` calls `clearTimeout(t)`. When unset, the controller is created but never armed (a **never-firing** signal) and `dispose()` is a no-op — with no time budget the default never cuts on time (backward-compat).
...
- **`maxToolCalls` moves into the budget — via TWO gate modes (top-of-round vs before-tool).** The handler consults the budget at the SAME points its inline checks fire today, but with the correct mode each place:
  ...
  - **`budget.canExecuteTool(state)` — prospectively before executing/surfacing a tool call** — count + time. The default cuts when `toolCallCount + 1 > maxToolCalls` (matching today's external-tool cap at controller-coordinator-handler.ts:1279 AND the internal cap), so a tool cannot exceed the cap. This replaces BOTH inline checks (the internal-tool gate and the external before-surface gate) with one prospective `canExecuteTool`. **Ordering (fixed, one model):** `canExecuteTool(state)` is consulted with the ALREADY-EXECUTED `toolCallCount` (BEFORE the increment) and cuts when `toolCallCount + 1 > maxToolCalls` — i.e. it decides whether the NEXT (about-to-run) tool call is allowed. Only if it returns `{continue:true}` does the handler then increment + persist `toolCallCount` and run/surface the call. (So the last legal call is not cut one early.)
...
- **Cancelled executor LLM must be detected explicitly (NOT the retry path) — cover BOTH shapes.** An aborted `executor.send` can surface two ways: (i) `makeSubagentClient` converts an aborted `llm.chat` into a RETURNED `SubagentResult { kind:'error', error:'…' }` (subagent-client.ts); (ii) a custom `ISubagentClient.send` (or the underlying provider) may THROW/REJECT on abort. The handler must handle both:
  - Wrap `deps.executor.send` in **try/catch**. In the CATCH, if `budget.signal.aborted` → `control-failure(reason:'step-timeout') → replan`; otherwise the existing unexpected-error handling (rethrow / degrade) is unchanged.
  - After a normal RETURN, check `budget.signal.aborted` FIRST — if aborted → `control-failure('step-timeout') → replan`, BEFORE the `res.kind==='error'` retry branch (control-coordinator-handler.ts:1212). A genuine provider error with the signal NOT aborted stays the retry path.
  `budget.signal.aborted` is the authoritative discriminator either way (not the error string).
...
- **MODIFY** `controller/controller-coordinator-handler.ts` (`ControllerHandlerDeps.callMcp` gains `signal?`; runStep: `beginStep` at entry, thread `budget.signal` into `executor.send` + `callMcp`, consult `budget.shouldContinueRound(state)` BEFORE each round (time/abort) AND `budget.canExecuteTool(state)` prospectively BEFORE each internal/external tool call (replacing both inline maxToolCalls checks), check `budget.signal.aborted` after `executor.send`/`callMcp` → `control-failure('step-timeout')` before the retry/escalate branches, map cut→control-failure→replan, `budget.dispose()` on step end; `ControllerHandlerDeps` gains `stepExecutionControl?`/`runExecutionControl?`).
