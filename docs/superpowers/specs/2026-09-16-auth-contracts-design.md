...
2. **The property: there is none, because a contract does not carry a credential at all.** Two earlier drafts of this item argued about *how* to put a secret in a config — widen `apiKey` or add a field beside it, then deprecate the old one. Both answered the wrong question.
   ...
   **`MakeLlmConfig` loses authentication rather than restating it.** An earlier draft made it a discriminated union so each branch could name its own credential; principle 9 rules that out, and the seam to use already exists: `withMainLlm(llm: ILlm)`, `withHelperLlm` and `withClassifierLlm` (`llm-agent-libs/src/builder.ts:246`, `:258`, `:264`) take an already-constructed provider. A consumer that needs one authenticated builds it — `new OpenAIProvider({ credential, model })` — and hands in the instance. `makeLlm` stays what it is: a convenience for the cases where nothing secret is involved, and `makeDefaultLlm` / `createDeepSeek` lose their plain-`string` parameter with it.

   **And that exposes the one place this costs something real.** `DefaultModelResolver` re-constructs a provider on every model switch, from a stored config: `makeLlm({ ...this.providerConfig, model: modelName })` (`providers.ts:311-326`), and `llm-agent-server-libs` does the same at `build-dag-coordinator-deps.ts:89`. A resolver that must construct **later** therefore needs whatever construction needs — which under principle 9 cannot be a stored secret. It takes a **factory** instead: `(model: string) => Promise<ILlm>`, supplied by the consumer, closing over the credential it already holds.
