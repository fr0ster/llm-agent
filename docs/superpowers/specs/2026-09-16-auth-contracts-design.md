...
2. **The property: there is none, because a contract does not carry a credential at all.** Two earlier drafts of this item argued about *how* to put a secret in a config — widen `apiKey` or add a field beside it, then deprecate the old one. Both answered the wrong question.
   ...
   **They cannot “take an instance or a factory” instead, though, and an earlier draft of this paragraph said they could.** These types *are* the YAML: the shipped template declares `llm: { provider, apiKey: ${DEEPSEEK_API_KEY}, model, temperature, classifierTemperature }` (`yaml-loader.ts:15-21`). A file holds no object and no function, so the fix is not to change what the field accepts but to **separate two layers that had been one**:
   ...
   |---|---|---|
   | **serializable app configuration** — `SmartServerLlmConfig`, `PipelineLlmProviderConfig`, the YAML | what a file can express: `provider`, `model`, `temperature`, `classifierTemperature`, `url` | **no secret, no instance** |
   | **runtime injection** — a separate options field on `SmartServer` | constructed `ILlm` instances per role, and an `IModelResolver` if the deployment wants switching | **instances, built by the app** |

   `apiKey` leaves the YAML altogether rather than becoming a variable name resolved later: the placeholder is substituted **into the loaded object**, which is exactly how a secret gets into a config type. The app reads the environment itself — which is what that placeholder always meant — builds the credential, constructs the provider, and passes the instance.
...
| `@mcp-abap-adt/sap-aicore-auth` (**new**) | `serviceKeyCredential(raw): IBearerCredential` — the existing `TokenProvider` and `parseServiceKey` moved out of `sap-aicore-embedder` with their tests, so both SAP packages and the composition root can use one implementation (§4.6.2) | new package |
...
+ new SapCoreAIProvider({ model, credential, serviceUrl: apiBaseUrl });
```
