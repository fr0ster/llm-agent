...
2. **The property, and precedence: add a property, never widen `apiKey`.** Adding `credential?: …` as a **new optional property** breaks nobody — a consumer that does not read it is unaffected. **Widening the existing `apiKey?: string` to `string | ICredential` breaks every consumer that reads it**, which is not a prediction: #306 measured the same move on `AnyLogger` and got `error TS2339: Property 'log' does not exist on type 'AnyLogger'`. So the existing fields are not touched, and the credential arrives beside them.

   **But “beside” is compatibility, not design, and an earlier draft of this section presented one as the other.** By §4's own argument a static key *is* a credential — `{ kind: 'api-key', secret: async () => 'sk' }` is three lines — so a config carrying both offers **two ways to do one job**, which is exactly what §7 is written against. We already pay that cost once, for the logger; agreeing to pay it again without saying so would be worse than the first time.

   So every existing field — `apiKey?: string` on the LLM providers, the embedders and `qdrant-rag`, `user`/`password` on the two databases, and the SAP `clientId`/`clientSecret` objects — is **deprecated on arrival and removed at the next major** (§9.11). Core ships the conversion so each call site is one line:
   ...
   **And the asymmetry with the logger makes this the easier case, which is worth saying rather than leaving implied.** `ILogger` cannot be converged now for a hard reason: llm-agent hands it *out* through `PipelineContext` and `IPipelinePlugin`, so renaming it breaks every consumer's plugin (§7, §9.9). Nothing hands `apiKey` out — it is an input and only an input — so the only thing keeping it is existing call sites, which a deprecation and a converter answer. Removing it in *this* release was considered and rejected on other grounds: §1.1 says a seam left unused changes nothing, and deleting the only documented way to authenticate would make adopting the new one mandatory rather than offered.
...
11. **One input for one job, at the next major.** This release adds `credential` beside `apiKey`, `user`/`password` and the SAP credential objects, and deprecates all of them; the next major removes them, leaving the credential as the only way to authenticate any provider. Scheduled rather than silently deferred, exactly as §9.9 schedules the logger — and for a weaker reason than that one: `apiKey` is an input only, so nothing but existing call sites keeps it, and `staticApiKey` / `staticLogin` make each of those a one-line change (§4.6.2).
