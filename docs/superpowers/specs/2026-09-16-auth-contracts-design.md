...
**Deletion has to reach the catalog, or hydration undoes it.** A record that outlives its collection is not a stale row, it is a resurrection: the next hydration adopts a collection whose data is gone and hands the caller something that looks valid. So `deleteCollection` removes the catalog record too, and the order is **record first, then the store's data**. Both failures leave something behind, and the question is only which — record first leaves an orphaned store, which is inert and reachable by nobody, while data first risks a record pointing at nothing, which comes back. Inert beats resurrected.

The failure semantics follow from that asymmetry rather than being invented:

- **The record's deletion fails** → the whole operation fails and the data is left alone. Proceeding would produce exactly the record-without-data case, so there is nothing to gain by continuing.
- **The record is gone and the data's deletion fails** → reported as today: the collection is unregistered, it is gone for the caller, and the orphaned store is named in the warning (`simple-rag-registry.ts:220-225`). Nothing will adopt it, because nothing records it.
- **`closeSession` needs no rule of its own.** It already collects its session's collections and calls `deleteCollection` for each, aggregating what failed into `SessionCloseIncompleteError` (`:324-334`), so it inherits both the record deletion and the semantics above. A session whose record deletions failed reports itself incomplete, which is what that error is for.
