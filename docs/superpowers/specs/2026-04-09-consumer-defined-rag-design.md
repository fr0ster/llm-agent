...
2. **`YamlPipeline`** (or `StructuredPipeline`) — configurable:
   - Parses YAML config or structured definition
...
// YAML with plugins
builder.setPipeline(
  new YamlPipeline('./pipeline.yaml', [companyDocsPlugin, sapNotesPlugin])
);
```
...
All registered stores are queried and written to in parallel.

### Plugin Interface

Plugins live inside pipeline implementations, not in builder. A plugin extends the pipeline with additional RAG stores and classifier knowledge.
...
builder.setPipeline(
  new YamlPipeline('./pipeline.yaml', [companyDocsPlugin])
);
```

Plugin registers stores and classifier extension. Pipeline decides when and how to query/upsert them. The default pipeline ignores plugins — only `YamlPipeline` (or similar) supports them.
...
| Component | Notes |
|-----------|-------|
| `IRag` interface | query/upsert/healthCheck — no changes |
...
2. Replace `withRag()` calls with `setToolsRag()` / `setHistoryRag()` or plugins
3. Replace `withRagUpsert(false)` with pipeline configuration
4. Add `setPipeline()` call (or rely on DefaultPipeline auto-selection)
5. If using custom stores — create plugins and use YamlPipeline
...
builder
  ...
  .setPipeline(new YamlPipeline('./pipeline.yaml', [plugin]))
  .build();
```
