...
## Approach — one transport-level idle keep-alive at the SSE surface
...
one), a single idle watchdog at the sole SSE surface fixes them all at once and stays
correct for pipelines added later.

The watchdog writes an SSE keep-alive comment whenever the client stream has been idle
longer than `heartbeatIntervalMs`; every real chunk resets it. It is pipeline-agnostic
— it does not know or care how any coordinator executes tools. The flat pipeline is
unaffected: its `value.heartbeat` chunks arrive every interval and keep the watchdog
reset, so it never fires there and flat behaviour is byte-for-byte unchanged.
...
export interface IdleHeartbeatOptions {
  /** Idle window in ms. `<= 0` disables (all methods become no-ops). */
  intervalMs: number;
...
re-arms. `stop()` cancels and makes further `reset()`/`stop()` no-ops. `intervalMs <=
0` → a no-op stub (keep-alive disabled). Timers are injectable so tests drive it
without real time.

### 2. `chat-route-handler` streaming branch consumes it

In the `if (body.stream)` branch (`chat-route-handler.ts:262+`):

- Read the interval as `cfg.agent?.heartbeatIntervalMs ?? 5000` — the same
  `agent.heartbeatIntervalMs` yaml key the tool-loop uses, so the cadence is
  consistent. **Note:** `resolveAgentSection`
  (`resolve-config-sections.ts:253-259`) already spreads `heartbeatIntervalMs`
  into `cfg.agent` at runtime, but the `SmartServerAgentConfig` interface
  (`smart-server.ts:188`) does not declare it. This change adds
  `heartbeatIntervalMs?: number` to that interface so the read is type-safe — no
  new parsing, just typing a field that is already populated.
- `const hb = createIdleHeartbeat({ intervalMs, onBeat: () => res.write(': keep-alive\n\n') });`
- Arm once before the loop (`hb.reset()`), then `hb.reset()` at the top of each
  `for await` iteration (any real chunk re-arms).
- `hb.stop()` in a `finally`, and on `res.on('close')` (client disconnect — never
  write to a dead socket).
- The existing `value.heartbeat` handling (`:304-308`) stays: on the flat path it
  writes `: heartbeat tool=… elapsed=…ms` and, being a chunk, also resets the
  watchdog — so the flat path keeps its richer per-tool comment and the watchdog
  never fires there.

The non-streaming (JSON) branch is untouched — there is no connection to keep alive.

## Data flow

```
streaming branch:
  hb = createIdleHeartbeat({ intervalMs, onBeat: () => res.write(': keep-alive\n\n') })
  hb.reset()                                   // arm
  res.on('close', () => hb.stop())             // client disconnect
  try {
    for await (const chunk of stream) {
      hb.reset()                               // real activity re-arms
      … existing chunk writes (content / heartbeat / tool_calls / [DONE]) …
    }
  } finally { hb.stop() }

long MCP call under linear/DAG/controller/stepper/cyclic:
...
- **Non-streaming request:** watchdog is created only inside the `body.stream` branch.
- **`heartbeatIntervalMs <= 0` or unset:** unset → default 5000; `<= 0` → disabled
  no-op stub (a deployment can turn keep-alive off).
...
- **Unit `sse-heartbeat.test.ts`** (injected manual scheduler — deterministic, no real
  ...
  is idempotent; `intervalMs <= 0` never beats.
- **Integration (chat-route streaming, short real interval e.g. 20ms):** a fake agent
  stream that stays idle > interval then yields one content chunk → the SSE output
  contains a `: keep-alive` line written **before** the content `data:` line; a
  flat-style stream that yields `value.heartbeat` chunks → output contains
  `: heartbeat tool=…` and **no** `: keep-alive` (watchdog stayed reset); on
  `res` close mid-idle, no write after close.
- **Regression:** existing `chat-route-handler` / streaming tests stay green (content,
  tool_calls, `[DONE]`, usage paths unchanged).
...
- **ARCHITECTURE / streaming:** the SSE surface emits an idle keep-alive comment every
  `heartbeatIntervalMs`, independent of pipeline; the flat pipeline additionally emits
  per-tool `: heartbeat …` comments.
...
- `smart-agent/http/sse-heartbeat.ts` — `createIdleHeartbeat` + types.
- `smart-agent/smart-server.ts` — declare `heartbeatIntervalMs?: number` on
  `SmartServerAgentConfig` (already populated by `resolveAgentSection`).
- `smart-agent/http/chat-route-handler.ts` — consume it in the streaming branch;
  `res.on('close')` stop; `finally` stop.
- `smart-agent/http/__tests__/sse-heartbeat.test.ts` (+ a streaming integration test).
