...
export class SmartAgent {
  ...
  async process(textOrMessages: string | Message[], options?: CallOptions): Promise<Result<SmartAgentResponse, OrchestratorError>> {
    ...
    try {
      ...
      if (result.ok) this.deps.logger?.log({ type: 'pipeline_done', traceId, stopReason: result.value.stopReason, iterations: result.value.iterations, toolCallCount: result.value.toolCallCount, durationMs });
      else this.deps.logger?.log({ type: 'pipeline_error', traceId, code: result.error.code, message: result.error.message, durationMs });
      return result;
  ...
  async *streamProcess(textOrMessages: string | Message[], options?: CallOptions & { externalTools?: any[] }): AsyncIterable<Result<LlmStreamChunk, OrchestratorError>> {
    ...
    try {
      ...
      if (options?.externalTools && options.externalTools.length > 0) {
        const validTools = options.externalTools.map(t => {
          if (t.name) return t;
  ...
  private async _preparePipeline(textOrMessages: string | Message[], opts: CallOptions | undefined, traceId: string, pipelineT0: number): Promise<Result<{ action: Subprompt; retrieved: { facts: RagResult[]; feedback: RagResult[]; state: RagResult[]; tools: McpTool[] }; messages: Message[]; toolClientMap: Map<string, IMcpClient>; isChat?: boolean }, OrchestratorError>> {
    ...
    if (this.deps.injectionDetector) {
      ...
      if (detection.detected) { logger?.log({ type: 'pipeline_error', traceId, code: 'PROMPT_INJECTION', message: `Injection detected: pattern=${detection.pattern ?? 'unknown'}`, durationMs: Date.now() - pipelineT0 }); return { ok: false, error: new OrchestratorError(`Prompt injection detected: pattern=${detection.pattern ?? 'unknown'}`, 'PROMPT_INJECTION') }; }
    }
    ...
    if (this.deps.helperLlm && history.length > summarizeLimit) { const summaryResult = await this._summarizeHistory(history, opts); if (summaryResult.ok) processedHistory = summaryResult.value; }
    ...
    await Promise.allSettled(others.map(async sp => { const store = ragStoreMap.get(sp.type); if (store) await store.upsert(sp.text, this._buildRagMetadata(), opts); }));
  ...
  private async *_runStreamingToolLoop(_action: Subprompt, retrieved: { facts: RagResult[]; feedback: RagResult[]; state: RagResult[]; tools: McpTool[] }, initialMessages: Message[], toolClientMap: Map<string, IMcpClient>, opts: CallOptions | undefined, traceId: string): AsyncIterable<Result<LlmStreamChunk, OrchestratorError>> {
    ...
    for (let iteration = 0; ; iteration++) {
      ...
      if (content || toolCalls.length > 0) messages = [...messages, { role: 'assistant', content: content || null, tool_calls: toolCalls.map(tc => ({ id: tc.id, type: 'function' as const, function: { name: tc.name, arguments: JSON.stringify(tc.arguments) } })) }];
      for (const toolCall of toolCalls) {
        ...
        else { const res = await client.callTool(toolCall.name, toolCall.arguments, opts); result = !res.ok ? res.error.message : typeof res.value.content === 'string' ? res.value.content : JSON.stringify(res.value.content); }
  ...
  private async _listAllTools(opts: CallOptions | undefined): Promise<{ tools: McpTool[]; toolClientMap: Map<string, IMcpClient> }> {
    ...
    for (const entry of settled) { if (entry.status === 'rejected' || !entry.value.result.ok) continue; for (const tool of entry.value.result.value) { if (!toolClientMap.has(tool.name)) { tools.push(tool); toolClientMap.set(tool.name, entry.value.client); } } }
  ...
  private async _runToolLoop(_action: Subprompt, retrieved: { facts: RagResult[]; feedback: RagResult[]; state: RagResult[]; tools: McpTool[] }, initialMessages: Message[], toolClientMap: Map<string, IMcpClient>, opts: CallOptions | undefined, traceId: string): Promise<Result<SmartAgentResponse, OrchestratorError>> {
    ...
    for (let iteration = 0; ; iteration++) {
      ...
      for (const toolCall of toolCalls) {
        ...
        const t0 = Date.now(); let result: string;
        if (this.deps.toolPolicy) { const verdict = this.deps.toolPolicy.check(toolCall.name); if (!verdict.allowed) { result = verdict.reason ?? `Tool "${toolCall.name}" blocked`; toolCallCount++; messages = [...messages, { role: 'tool', content: result, tool_call_id: toolCall.id }]; continue; } }
        const client = toolClientMap.get(toolCall.name);
        if (!client) result = `Tool "${toolCall.name}" not found`;
        else { const res = await client.callTool(toolCall.name, toolCall.arguments, opts); result = !res.ok ? res.error.message : typeof res.value.content === 'string' ? res.value.content : JSON.stringify(res.value.content); }
        toolCallCount++; messages = [...messages, { role: 'tool', content: result, tool_call_id: toolCall.id }];
      }
    }
  }
  ...
  private async _summarizeHistory(history: Message[], opts?: CallOptions): Promise<Result<Message[], OrchestratorError>> {
    ...
    const toSummarize = history.slice(0, -5); const recent = history.slice(-5);
