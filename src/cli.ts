#!/usr/bin/env node
/**
 * CLI Test Launcher for LLM Agent
 * 
...
if (existsSync(envPath)) {
  ...
  console.log('💡 Tip: Create .env file from .env.template for easier configuration\n');
}
...
console.log(`🤖 LLM Agent Test Launcher v${packageJson.version}\n`);

// Get configuration from environment
const llmProvider = process.env.LLM_PROVIDER || 'openai';
const mcpEndpoint = process.env.MCP_ENDPOINT || 'http://localhost:4004/mcp/stream/http';
const mcpDisabled = process.env.MCP_DISABLED === 'true' || process.argv.includes('--llm-only') || process.argv.includes('--no-mcp');

// Get test message from command line args or use default
// Filter out flags from message
const messageArgs = process.argv.slice(2).filter(arg => !arg.startsWith('--'));
const testMessage = messageArgs[0] || (mcpDisabled ? 'Hello! Can you introduce yourself?' : 'What tools are available?');
...
async function main() {
  try {
    ...
    if (llmProvider === 'openai') {
      ...
      if (anthropicKey) {
        const masked = anthropicKey.substring(0, 7) + '...' + anthropicKey.substring(anthropicKey.length - 4);
        console.log(`   API Key: ${masked} ✅`);
      } else {
        console.log(`   API Key: ❌ NOT SET`);
      }
      ...
      if (deepseekKey) {
        const masked = deepseekKey.substring(0, 7) + '...' + deepseekKey.substring(deepseekKey.length - 4);
        console.log(`   API Key: ${masked} ✅`);
      } else {
        console.log(`   API Key: ❌ NOT SET`);
      }
    ...
    // Create LLM provider
    let llmProviderInstance: OpenAIProvider | AnthropicProvider | DeepSeekProvider;
    ...
    switch (llmProvider.toLowerCase()) {
      case 'openai': {
        ...
        const orgInfo = process.env.OPENAI_ORG ? ` (org: ${process.env.OPENAI_ORG.substring(0, 8)}...)` : '';
        const projectInfo = (process.env.OPENAI_PROJECT || process.env.OPENAI_PRJ) ? ` (project: ${(process.env.OPENAI_PROJECT || process.env.OPENAI_PRJ)!.substring(0, 8)}...)` : '';
      ...
      case 'deepseek': {
        ...
        if (process.env.DEEPSEEK_ENDPOINT) {
          baseURL = process.env.DEEPSEEK_ENDPOINT.replace('/chat/completions', '');
        }
      ...
      case 'ollama': {
        ...
        throw new Error('Ollama provider is not yet implemented. Use: openai, anthropic, or deepseek');
      }
      
      default:
        throw new Error(`Unsupported LLM provider: ${llmProvider}. Use: openai, anthropic, or deepseek`);
    }
    ...
    if (response.toolResults && response.toolResults.length > 0) {
      ...
      response.toolResults.forEach((tr) => {
        if (tr.error) {
          ...
          const resultStr = JSON.stringify(tr.result).substring(0, 100);
          console.log(`   - ${tr.name}: ✅ ${resultStr}${JSON.stringify(tr.result).length > 100 ? '...' : ''}`);
        }
      });
    }
    console.log('─'.repeat(60));
