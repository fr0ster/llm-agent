  ...
  protected async *streamLLMWithTools(
    ...
    yield* this.streamOpenAICompatible(
      ...
      { Authorization: `Bearer ${provider.config.apiKey as string}` },
      {
        ...
        tools: (() => { const f = this.convertToolsToOpenAIFunctions(tools); return f.length > 0 ? f : undefined; })(),
        tool_choice: this.convertToolsToOpenAIFunctions(tools).length > 0 ? 'auto' : undefined,
