        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (!res.ok) {
                    ...
                    throw new RagError(`OpenAI embed error: HTTP ${res.status} - ${errorText}`, 'EMBED_ERROR');
                }
        ...
        for (let start = 0; start < texts.length; start += chunkSize) {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    if (!res.ok) {
                        const errorText = await res.text();
                        throw new RagError(`OpenAI batch embed error: HTTP ${res.status} - ${errorText}`, 'EMBED_ERROR');
                    }
                    const json = (await res.json());
