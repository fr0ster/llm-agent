        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (!res.ok) {
                    ...
                    throw new RagError(`Ollama embed error: HTTP ${res.status} - ${errorText}`, 'EMBED_ERROR');
                }
    ...
    async embedBatch(texts, options) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (!res.ok) {
                    const errorText = await res.text();
                    throw new RagError(`Ollama batch embed error: HTTP ${res.status} - ${errorText}`, 'EMBED_ERROR');
                }
                const json = (await res.json());
