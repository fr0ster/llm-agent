...
test(
  '#213 per-session (default): concurrent acquire returns DISTINCT client instances; callTool returns session-specific tokens — no crossing',
  async () => {
...
test(
  '#213 shared opt-out: mcpSharedClient=true → buildPerSessionMcpClients never called; both sessions share the SAME client instance',
  async () => {
    const ragRegistry = makeRagRegistry();
    ...
    const lc = buildSessionLifecycle({
      ...
      buildPerSessionMcpClients: () => {
        buildCalled++;
        return { clients: [makeFakeClient('per-session-token')], close: async () => {} };
      },
      buildAgent: async (parts: SessionAgentParts) => {
        capturedClients.push(parts.mcpClients as IMcpClient[]);
        return undefined;
...
test(
  '#213 discrimination proof: shared wiring causes per-session "distinct" assertion to FAIL — confirms the first test guards the real isolation',
  async () => {
