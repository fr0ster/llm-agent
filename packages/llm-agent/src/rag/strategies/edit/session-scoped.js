...
export class SessionScopedEditStrategy {
    ...
    async upsert(text, metadata, options) {
        const stamped = {
            ...
            createdAt: typeof metadata.createdAt === 'number'
                ? metadata.createdAt
                : Date.now(),
        };
        let id;
        ...
        const res = await this.writer.upsertRaw(id, text, { ...stamped, id }, options);
    ...
    async clear() {
        if (this.writer.clearAll)
            return this.writer.clearAll();
