...
const EXPAND_SYSTEM_PROMPT = 'Given the user query, produce an expanded search query with synonyms and related terms. Return ONLY the expanded query text, no explanation.';
export class LlmQueryExpander {
    ...
    async expand(query, options) {
        try {
            ...
            return {
                ok: false,
                error: new RagError(`Query expansion failed: ${String(err)}`, 'QUERY_EXPAND_ERROR'),
            };
        }
    }
}
