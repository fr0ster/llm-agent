...
export class SimpleRagRegistry {
    ...
    async createCollection(params) {
        if (!this.providerRegistry) {
            return {
                ok: false,
                error: new RagError('No IRagProviderRegistry configured on SimpleRagRegistry', 'RAG_NO_PROVIDER_REGISTRY'),
            };
        }
        ...
        if (this.entries.has(params.collectionName)) {
            return {
                ok: false,
                error: new RagError(`Collection '${params.collectionName}' already exists`, 'RAG_DUPLICATE_COLLECTION'),
            };
        }
        ...
        if (!registered) {
            return {
                ok: false,
                error: new RagError(`Collection '${params.collectionName}' vanished after registration`, 'RAG_REGISTER_FAILED'),
            };
        }
