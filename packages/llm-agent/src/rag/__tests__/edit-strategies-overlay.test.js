...
import { OverlayEditStrategy, SessionScopedEditStrategy, } from '../strategies/edit/index.js';
...
describe('SessionScopedEditStrategy', () => {
    it('stamps sessionId on every write and uses the session id strategy', async () => {
        const { writer, rows } = fakeWriter();
        const ed = new SessionScopedEditStrategy(writer, 'S', new SessionScopedIdStrategy('S'));
        const res = await ed.upsert('v', { id: 'x' });
        assert.ok(res.ok);
