...
import { FallbackQueryEmbedding, QueryEmbedding, TextOnlyEmbedding, } from '../query-embedding.js';
// ---------------------------------------------------------------------------
// Stub embedder factory
// ---------------------------------------------------------------------------
function makeStubEmbedder(usage) {
    let callCount = 0;
    return {
        get callCount() {
            return callCount;
...
describe('TextOnlyEmbedding', () => {
    ...
    it('toVector() rejects with RagError', async () => {
        ...
        await assert.rejects(() => te.toVector(), (err) => {
    ...
    it('has no getUsage method', () => {
        const te = new TextOnlyEmbedding('query');
        assert.equal(typeof te.getUsage, 'undefined');
    });
});
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=query-embedding.test.js.map