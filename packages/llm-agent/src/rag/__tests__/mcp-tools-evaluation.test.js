...
import { Bm25OnlyStrategy, RrfStrategy, VectorOnlyStrategy, WeightedFusionStrategy, } from '../search-strategy.js';
...
const GOLDEN_QUERIES = [
    ...
    // --- 3. Ambiguous/hard queries ---
    {
        ...
        k: 3,
        description: 'should prefer GetTableContents over ReadTable (data vs definition)',
    },
    {
...
function buildVectorRagFactory(strategyName) {
    return {
        name: strategyName,
        factory: () => {
...
for (const stratDef of STRATEGIES) {
    describe(`MCP Tools Evaluation — ${stratDef.name}`, () => {
        it('positive queries find expected tool in top-k', async () => {
            ...
            for (const gq of positiveQueries) {
                ...
                for (const expectedId of gq.expectedTopIds ?? []) {
                    assert.ok(topIds.includes(expectedId), `[${stratDef.name}] Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]${gq.description ? ` (${gq.description})` : ''}`);
                }
            }
        });
            ...
            for (const gq of negativeQueries) {
                ...
                if (results.length > 0) {
                    ...
                    for (const absentId of gq.expectedAbsentIds ?? []) {
                        assert.notEqual(topId, absentId, `[${stratDef.name}] Query "${gq.query}": "${absentId}" should NOT be top-1${gq.description ? ` (${gq.description})` : ''}`);
                    }
                }
            }
        });
        ...
        it('recall for multi-result queries at specified k', async () => {
            ...
            const singleResultQueries = GOLDEN_QUERIES.filter((q) => q.expectedTopIds && q.expectedTopIds.length === 1);
            for (const gq of singleResultQueries) {
                const results = await runQuery(rag, gq.query, gq.k, embedder);
                const r = recall(results, gq.expectedTopIds ?? []);
                assert.equal(r, 1.0, `[${stratDef.name}] Query "${gq.query}": recall = ${r}, expected 1.0`);
            }
        });
    });
}
...
// ---------------------------------------------------------------------------
class MockTranslatePreprocessor {
    name = 'mock-translate';
...
describe('MCP Tools Evaluation — VectorRag + MockTranslatePreprocessor (RRF)', () => {
    ...
    it('positive queries find expected tool in top-k', async () => {
        ...
        for (const gq of positiveQueries) {
            ...
            const topIds = results.map((r) => r.metadata.id);
            for (const expectedId of gq.expectedTopIds ?? []) {
                assert.ok(topIds.includes(expectedId), `[Translate+RRF] Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]`);
            }
        }
    });
...
//# sourceMappingURL=mcp-tools-evaluation.test.js.map