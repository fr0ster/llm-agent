...
describe('InMemoryRag', () => {
    ...
    describe('top-k ordering', () => {
        it('returns results sorted by score descending', async () => {
            ...
            for (let i = 0; i < result.value.length - 1; i++) {
                assert.ok(result.value[i].score >= result.value[i + 1].score, `score[${i}]=${result.value[i].score} < score[${i + 1}]=${result.value[i + 1].score}`);
            }
    ...
    describe('AbortSignal upsert', () => {
        it('returns ABORTED error when signal is pre-aborted', async () => {
            ...
            controller.abort();
            const result = await rag.upsert('some text', {}, { signal: controller.signal });
            assert.ok(!result.ok);
...
describe('InMemoryRag backend writer', () => {
    ...
    it('clearAll empties the store', async () => {
        ...
        assert.equal((await rag.getById?.('x'))?.ok && (await rag.getById?.('x'))?.value, null);
    });
});
