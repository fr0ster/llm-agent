...
import { Bm25OnlyStrategy, RrfStrategy, VectorOnlyStrategy, } from '../search-strategy.js';
...
// Metric helpers
// ---------------------------------------------------------------------------
function precision(results, expectedIds) {
    if (results.length === 0)
        return 0;
    const hits = results.filter((r) => expectedIds.includes(r.metadata.id));
    return hits.length / results.length;
}
function reciprocalRank(results, expectedIds) {
    for (let i = 0; i < results.length; i++) {
        if (expectedIds.includes(results[i].metadata.id)) {
            return 1 / (i + 1);
        }
    }
    return 0;
}
function recall(results, expectedIds) {
    if (expectedIds.length === 0)
        return 1;
    const found = expectedIds.filter((id) => results.some((r) => r.metadata.id === id));
...
describe('RAG Evaluation — InMemoryRag', () => {
    it('all golden queries return expected tool in top-k', async () => {
        ...
        for (const gq of GOLDEN_QUERIES) {
            ...
            for (const expectedId of gq.expectedTopIds) {
                assert.ok(topIds.includes(expectedId), `Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]`);
            }
        }
    });
    ...
    it('top-1 precision for tool lookups >= 80%', async () => {
        const rag = new InMemoryRag({ dedupThreshold: 0.99 });
        await seedRag(rag, GOLDEN_CORPUS);
        const toolQueries = GOLDEN_QUERIES.filter((gq) => gq.expectedTopIds.some((id) => id.startsWith('tool:')));
        let totalP1 = 0;
        ...
        assert.ok(avgP1 >= 0.8, `Top-1 precision = ${(avgP1 * 100).toFixed(1)}%, expected >= 80%`);
    });
...
describe('RAG Evaluation — VectorRag (hybrid)', () => {
    ...
    it('BM25 component boosts exact-term matches', async () => {
        ...
        const exactResult = await runQuery(rag, 'abap_get_object_source', 3, embedder);
        const synonymResult = await runQuery(rag, 'retrieve program listing', 3, embedder);
        const exactTopId = exactResult[0]?.metadata.id;
        // Exact query must return the right tool as #1
        assert.equal(exactTopId, 'tool:abap_get_object_source', `Exact query top-1 should be tool:abap_get_object_source, got ${exactTopId}`);
        // Exact query score should be higher than synonym query score for the same doc
        const exactScore = exactResult.find((r) => r.metadata.id === 'tool:abap_get_object_source')
            ?.score ?? 0;
        const synonymScore = synonymResult.find((r) => r.metadata.id === 'tool:abap_get_object_source')
            ?.score ?? 0;
        assert.ok(exactScore > synonymScore, `Exact-term score (${exactScore.toFixed(3)}) should exceed synonym score (${synonymScore.toFixed(3)})`);
    });
});
// ---------------------------------------------------------------------------
// Tests — VectorRag (RRF)
...
describe('RAG Evaluation — VectorRag (RRF)', () => {
    ...
    it('all golden queries return expected tool in top-k', async () => {
        ...
        for (const gq of GOLDEN_QUERIES) {
            ...
            const topIds = results.map((r) => r.metadata.id);
            for (const expectedId of gq.expectedTopIds) {
                assert.ok(topIds.includes(expectedId), `[RRF] Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]`);
            }
        }
    });
    it('MRR >= 0.7 across all golden queries', async () => {
...
describe('RAG Evaluation — VectorRag (vector-only)', () => {
    ...
    it('all golden queries return expected tool in top-k', async () => {
        ...
        for (const gq of GOLDEN_QUERIES) {
            ...
            const topIds = results.map((r) => r.metadata.id);
            for (const expectedId of gq.expectedTopIds) {
                assert.ok(topIds.includes(expectedId), `[Vector-only] Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]`);
            }
        }
    });
    it('MRR >= 0.5 across all golden queries', async () => {
        ...
        const mrr = totalRR / GOLDEN_QUERIES.length;
        assert.ok(mrr >= 0.5, `[Vector-only] MRR = ${mrr.toFixed(3)}, expected >= 0.5`);
    });
});
// ---------------------------------------------------------------------------
...
describe('RAG Evaluation — VectorRag (BM25-only)', () => {
    ...
    it('all golden queries return expected tool in top-k', async () => {
        ...
        for (const gq of GOLDEN_QUERIES) {
            ...
            const topIds = results.map((r) => r.metadata.id);
            for (const expectedId of gq.expectedTopIds) {
                assert.ok(topIds.includes(expectedId), `[BM25-only] Query "${gq.query}": expected "${expectedId}" in top-${gq.k}, got [${topIds.join(', ')}]`);
            }
        }
    });
    it('MRR >= 0.5 across all golden queries', async () => {
        ...
        const mrr = totalRR / GOLDEN_QUERIES.length;
        assert.ok(mrr >= 0.5, `[BM25-only] MRR = ${mrr.toFixed(3)}, expected >= 0.5`);
    });
});
//# sourceMappingURL=rag-evaluation.test.js.map