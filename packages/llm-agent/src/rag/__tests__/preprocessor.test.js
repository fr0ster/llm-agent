...
import { ExpandPreprocessor, NoopDocumentEnricher, NoopQueryPreprocessor, PreprocessorChain, TranslatePreprocessor, } from '../preprocessor.js';
...
describe('ExpandPreprocessor', () => {
    it('expands query with LLM-generated synonyms', async () => {
        ...
        assert.ok(result.ok);
        assert.equal(result.value, 'create transport transport request workbench customizing');
    });
...
describe('PreprocessorChain', () => {
    it('runs preprocessors in sequence', async () => {
        const translate = new TranslatePreprocessor(makeLlm([{ content: 'read dumps' }]));
        const expand = new ExpandPreprocessor(makeLlm([{ content: 'runtime feeds' }]));
