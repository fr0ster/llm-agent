...
describe('OpenAiApiAdapter.normalizeRequest', () => {
    ...
    it('parses a valid request with defaults', () => {
        ...
        assert.match(result.context.protocol.id, /^chatcmpl-/);
    ...
    it('throws AdapterValidationError for non-object body', () => {
        assert.throws(() => adapter.normalizeRequest('not an object'), AdapterValidationError);
    });
    ...
    it('throws AdapterValidationError when messages is missing', () => {
        assert.throws(() => adapter.normalizeRequest({ model: 'gpt-4' }), AdapterValidationError);
    });
    it('throws AdapterValidationError when messages is empty', () => {
        assert.throws(() => adapter.normalizeRequest({ messages: [] }), AdapterValidationError);
    });
});
// ---------------------------------------------------------------------------
// formatResult
...
describe('OpenAiApiAdapter.transformStream', () => {
    ...
    it('emits correct chunk sequence with [DONE]', async () => {
        ...
        const first = parseData(events[0]);
        const firstChoices = first.choices;
        const firstDelta = firstChoices[0].delta;
        ...
        const second = parseData(events[1]);
        const secondChoices = second.choices;
        const secondDelta = secondChoices[0].delta;
        assert.equal(secondDelta.content, ' world');
        assert.equal(secondDelta.role, undefined);
        // Third chunk — finish_reason
        const third = parseData(events[2]);
        const thirdChoices = third.choices;
    ...
    it('emits usage chunk before [DONE]', async () => {
        ...
        const usageEvent = parseData(events[events.length - 2]);
    ...
    it('handles error chunks with finish_reason stop', async () => {
        ...
        const first = parseData(events[0]);
        const firstChoices = first.choices;
        assert.equal(firstChoices[0].delta.content, 'Start');
        // Error chunk has finish_reason: 'stop'
        const errorEvent = parseData(events[1]);
        const errorChoices = errorEvent.choices;
        assert.equal(errorChoices[0].finish_reason, 'stop');
        assert.ok(errorChoices[0].delta.content.includes('LLM failed'));
        // [DONE] at the end
        assert.equal(events[events.length - 1].data, '[DONE]');
    });
    it('emits tool_calls delta in first chunk', async () => {
        ...
        const first = parseData(events[0]);
        const firstChoices = first.choices;
        const delta = firstChoices[0].delta;
    ...
    it('emits default finish_reason stop when stream has no finishReason', async () => {
        ...
        assert.equal(events.length, 3);
        const finishEvent = parseData(events[1]);
        const choices = finishEvent.choices;
        assert.equal(choices[0].finish_reason, 'stop');
    });
});
