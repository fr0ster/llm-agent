...
describe('AnthropicApiAdapter.normalizeRequest', () => {
    ...
    it('extracts system content blocks as system message', () => {
        ...
        assert.equal(result.messages.length, 2);
        assert.equal(result.messages[0].role, 'system');
        assert.equal(result.messages[0].content, 'First instruction.\nSecond instruction.');
    });
    ...
    it('converts assistant tool_use blocks', () => {
        ...
        assert.equal(msg.tool_calls?.[0].function.arguments, JSON.stringify({ city: 'NYC' }));
    });
    ...
    it('throws AdapterValidationError when messages is missing', () => {
        assert.throws(() => adapter.normalizeRequest({ model: 'claude-sonnet-4-20250514' }), AdapterValidationError);
    });
