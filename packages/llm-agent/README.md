...
- Library helpers usable when embedding SmartAgent in your own server:
  - **Resilience**: `CircuitBreaker`, `CircuitBreakerLlm`, `FallbackRag`, and the embedder chain — `BatchChunkingEmbedder`, `RetryEmbedder`/`withRetry`, `CircuitBreakerEmbedder`/`withCircuitBreaker`, `composeResilientEmbedder` (all capability-preserving factories)
