...
import { assertCollectionName, createExtensionSql, createTableSql, quoteIdent, } from './schema.js';
...
export class PgVectorRag {
    ...
    async getById(id, options) {
        ...
        try {
            await this.maybeEnsureSchema();
            const client = await this.clientPromise;
            const { rows } = await client.query(`SELECT id, text, metadata FROM ${quoteIdent(this.collectionName)} WHERE id = $1`, [id]);
            const row = rows[0];
            if (!row)
        ...
        return {
            ...
            deleteByIdRaw: async (id) => {
                try {
                    await this.maybeEnsureSchema();
                    const client = await this.clientPromise;
                    const res = await client.query(`DELETE FROM ${quoteIdent(this.collectionName)} WHERE id = $1`, [id]);
