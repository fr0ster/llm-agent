...
export class PgVectorRagProvider extends AbstractRagProvider {
    ...
    async listCollections() {
        try {
            ...
            const { rows } = await client.query('SELECT table_name FROM information_schema.tables WHERE table_schema = $1', [schema]);
    ...
    requireClient() {
        if (!this.clientFactory) {
            throw new Error('PgVectorRagProvider deleteCollection/listCollections require clientFactory; provide one to enable schema-level operations');
        }
