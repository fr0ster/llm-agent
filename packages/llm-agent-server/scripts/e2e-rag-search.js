...
import { IntentToolIndexing, OriginalToolIndexing, SynonymToolIndexing, } from '../src/smart-agent/rag/tool-indexing-strategy.js';
...
async function prepareEntries(tool, strategies) {
...
async function main() {
    ...
    for (const tc of CASES) {
        ...
        for (const sc of stores) {
            ...
            const ids = results.map((r) => extractToolName(r.metadata.id));
            const hit = tc.expectAny.some((e) => ids.includes(e));
            if (hit)
                scores[sc.name]++;
            const top3 = results
                .slice(0, 3)
                .map((r) => `${extractToolName(r.metadata.id)}(${r.score.toFixed(3)})`)
                .join(', ');
...
//# sourceMappingURL=e2e-rag-search.js.map