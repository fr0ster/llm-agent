...
// Fixtures
// ---------------------------------------------------------------------------
function makeFact(text, score) {
    return { text, score, metadata: {} };
}
function makeTool(name, description) {
    return { name, description, inputSchema: {} };
}
function makeToolRecord(name, content) {
    return {
        call: { id: `${name}-1`, name, arguments: {} },
        result: { content },
    };
}
const ACTION = { type: 'action', text: 'What should I do next?' };
const EMPTY = { ragResults: {}, tools: [] };
// ---------------------------------------------------------------------------
// Empty retrieved
// ---------------------------------------------------------------------------
...
describe('ContextAssembler — facts only', () => {
    it('system message contains ## Known Facts', async () => {
        const assembler = new ContextAssembler();
        const r = await assembler.assemble(ACTION, {
...
describe('ContextAssembler — tools only', () => {
    it('tools-only input produces no tool descriptions in system prompt', async () => {
        const assembler = new ContextAssembler();
        const r = await assembler.assemble(ACTION, { ragResults: {}, tools: [makeTool('search', 'Search the web')] }, []);
        assert.ok(r.ok);
...
describe('ContextAssembler — provenance', () => {
    it('provenance off by default — no score annotations', async () => {
        const assembler = new ContextAssembler();
        const r = await assembler.assemble(ACTION, { ragResults: { facts: [makeFact('Some fact', 0.92)] }, tools: [] }, []);
        assert.ok(r.ok);
        const sys = r.value.find((m) => m.role === 'system');
        assert.ok(sys);
        assert.ok(!sys.content.includes('[score:'), 'should not include score annotation');
    });
    it('provenance on — score annotations present', async () => {
        ...
        const sys = r.value.find((m) => m.role === 'system');
        assert.ok(sys);
        assert.ok(sys.content.includes('[score: 0.92]'), 'should include score annotation');
    });
});
// ---------------------------------------------------------------------------
// Snapshot test
...
describe('ContextAssembler — snapshot', () => {
    it('fixed input produces exact Message[] structure', async () => {
        ...
        const r = await assembler.assemble({ type: 'action', text: 'Calculate 2+2.' }, {
...
describe('ContextAssembler — token budget', () => {
    ...
    it('tools overflow — tools at end of list dropped first', async () => {
        ...
        const r = await assembler.assemble({ type: 'action', text: 'Hi' }, {
            ragResults: {},
            tools: [
                makeTool('first-tool', 'First tool description that is quite long for budget'),
                makeTool('second-tool', 'Second tool description also quite long for budget'),
                makeTool('third-tool', 'Third tool description that overflows the token budget'),
            ],
        }, []);
        assert.ok(r.ok);
        const sys = r.value.find((m) => m.role === 'system');
        ...
        if (sys) {
            ...
            if (hasFirstTool) {
                ...
                assert.ok(!hasThirdTool, 'last tool should be dropped before first tool');
            }
        }
    ...
    it('facts overflow — facts with lowest score dropped', async () => {
        ...
        if (sys) {
            ...
            if (hasHigh) {
                assert.ok(!hasLow, 'low score fact should be dropped before high score fact');
            }
        }
    });
    it('action always kept — user message preserved even when everything else dropped', async () => {
        ...
        const r = await assembler.assemble({ type: 'action', text: 'Critical action text' }, {
            ragResults: {
                facts: [
                    makeFact('Some fact that is long and will not fit in budget', 0.5),
                ],
                feedback: [
                    makeFact('Some feedback that is long and will not fit in budget', 0.5),
                ],
                state: [
...
describe('ContextAssembler — systemPromptPreamble', () => {
    ...
    it('preamble and context sections joined with double newline', async () => {
        ...
        const r = await assembler.assemble(ACTION, { ragResults: { facts: [makeFact('Fact here', 0.9)] }, tools: [] }, []);
        assert.ok(r.ok);
        const sys = r.value.find((m) => m.role === 'system');
        assert.ok(sys);
        assert.ok(sys.content.includes('Preamble text.'), 'should contain preamble');
        assert.ok(sys.content.includes('## Known Facts'), 'should contain context section');
        // Preamble should come before context sections
        assert.ok(sys.content.indexOf('Preamble text.') <
            sys.content.indexOf('## Known Facts'), 'preamble should precede context');
    });
});
// ---------------------------------------------------------------------------
// Recent Actions
...
describe('ContextAssembler — recent actions', () => {
    ...
    it('no ## Recent Actions when recentActions empty', async () => {
        const assembler = new ContextAssembler();
        const r = await assembler.assemble(ACTION, { ragResults: {}, tools: [], recentActions: [] }, []);
        assert.ok(r.ok);
        const sys = r.value.find((m) => m.role === 'system');
        if (sys) {
            assert.ok(!sys.content.includes('## Recent Actions'));
