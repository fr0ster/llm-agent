...
describe('Mixed internal + external tool calls', () => {
    it('executes internal tools and returns external tool calls to client', async () => {
        ...
        const mcpClient = makeMcpClient([{ name: 'ReadClass', description: 'Read ABAP class', inputSchema: {} }], new Map([['ReadClass', { content: 'class ZCL_TEST { method1() {} }' }]]));
        ...
        const toolMsg = capturedMessages.find((m) => m.role === 'tool' && m.tool_call_id === 'call_1');
        assert.ok(toolMsg, 'internal tool result should be in context');
        assert.ok(toolMsg.content?.includes('ZCL_TEST'), 'ReadClass result should contain class content');
    });
    it('external-only calls still work (no pending results)', async () => {
        const streamLlm = {
            async chat() {
                return {
                    ok: true,
                    value: { content: '', finishReason: 'tool_calls' },
                };
            },
            async *streamChat() {
                yield {
    ...
    it('internal-only calls execute normally (no external)', async () => {
        const mcpClient = makeMcpClient([{ name: 'ReadClass', description: 'Read', inputSchema: {} }], new Map([['ReadClass', { content: 'class content' }]]));
        const { deps } = makeDefaultDeps({
            llmResponses: [
                {
                    content: 'reading',
        ...
        assert.ok(r.ok);
        assert.ok(r.value.content.includes('done'), 'response should contain final content');
        assert.equal(mcpClient.callCount, 1);
    });
});
//# sourceMappingURL=parallel-mixed-tool-calls.test.js.map