...
function makeClientWithoutHealthCheck() {
...
function makeSuccessFactory(client) {
    let callCount = 0;
    let closeCalls = 0;
...
describe('LazyConnectionStrategy', () => {
    it('returns current clients when all healthy', async () => {
        ...
        const strategy = new LazyConnectionStrategy([httpConfig], undefined, factory);
        ...
        assert.ok(client.healthCallCount > 0, 'healthCheck should have been called');
    });
    ...
    it('respects cooldown — factory called only once when cooldown not expired', async () => {
        ...
        const strategy = new LazyConnectionStrategy([httpConfig], { cooldownMs: 60000 }, factory);
    ...
    it('reconnects only the failed endpoint when one slot is healthy and one is missing', async () => {
        ...
        const strategy = new LazyConnectionStrategy([httpConfig, http2Config], { cooldownMs: 0 }, factory);
    ...
    it('returns toolsChanged: false when skipRevectorize: true', async () => {
        const factory = makeSuccessFactory();
        const strategy = new LazyConnectionStrategy([httpConfig], { skipRevectorize: true }, factory);
        const result = await strategy.resolve([]);
    ...
    it('falls back to listTools() when healthCheck is not available', async () => {
        ...
        await strategy.resolve([]);
        assert.equal(client.listToolsCallCount, listCallsBefore + 1, 'listTools should be called as health fallback');
        // Factory should not have been called again
        assert.equal(factory.callCount, 1);
    });
    it('does not throw when factory fails — returns empty clients', async () => {
        const factory = makeFailingFactory();
        const strategy = new LazyConnectionStrategy([httpConfig], { cooldownMs: 0 }, factory);
        // Should never throw
...
//# sourceMappingURL=lazy-connection-strategy.test.js.map