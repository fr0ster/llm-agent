...
describe('External tools — [client-provided] description prefix', () => {
    it('prefixes description in direct format', () => {
        ...
        assert.equal(tools[0].description, '[client-provided] Write a file to disk');
    });
    ...
    it('does not double-prefix on re-normalization', () => {
        ...
        assert.equal(second[0].description, '[client-provided] desc', 'second normalization must not add another prefix');
    });
...
describe('External tools — system prompt priority instruction', () => {
    it('injects priority instruction into system message when external tools present', async () => {
        ...
        assert.ok(sys.content?.includes('Always prefer internal tools'), 'should contain priority instruction');
        assert.ok(sys.content?.includes('[client-provided]'), 'should mention client-provided marker');
    });
    it('does not inject priority instruction when no external tools', async () => {
        ...
        const sys = capturedMessages.find((m) => m.role === 'system');
        assert.ok(sys, 'system message should exist');
        assert.ok(!sys.content?.includes('Always prefer internal tools'), 'should NOT contain priority instruction without external tools');
    });
});
