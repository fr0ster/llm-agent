...
describe('Per-iteration RAG tool re-selection', () => {
     ...
     */
    function makeToolCallThenStopLlm(toolName, _toolResultContent, onSecondCall) {
        let callCount = 0;
        return {
            get callCount() {
                return callCount;
    ...
    it('does not re-select tools when toolReselectPerIteration is false (default)', async () => {
        ...
        assert.equal(toolsRag.queryCalls?.length ?? 0, 0, 'tools RAG should not be queried when toolReselectPerIteration is off');
    });
...
//# sourceMappingURL=tool-reselection.test.js.map