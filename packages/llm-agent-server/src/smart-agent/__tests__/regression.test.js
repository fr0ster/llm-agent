...
import { makeClassifier, makeDefaultDeps, makeLlm, makeMcpClient, makeMetadataRag, makeRag, } from '../testing/index.js';
    ...
    it('assembles partial arguments and executes tool with merged JSON object', async () => {
        ...
        const mcpClient = {
            ...
            async callTool(_name, args) {
                capturedArgs = args;
                return { ok: true, value: { content: '3' } };
            },
        };
        const { deps } = makeDefaultDeps({
...
describe('Regression — duplicate tool name from two clients: first wins', () => {
    it('second client toolA never called', async () => {
        const client1 = makeMcpClient([{ name: 'toolA', description: 'A from client1', inputSchema: {} }], new Map([['toolA', { content: 'client1 result' }]]));
        const client2 = makeMcpClient([{ name: 'toolA', description: 'A from client2', inputSchema: {} }], new Map([['toolA', { content: 'client2 result' }]]));
        const { deps } = makeDefaultDeps({
...
describe('Regression — context assembled once before tool loop', () => {
    it('assembler is called once; tool loop continues without re-assembly', async () => {
        ...
        const flakyAssembler = {
            async assemble(_action, _retrieved, _toolResults, _opts) {
                assemblerCallCount++;
                return {
                    ok: true,
...
describe('Regression — helperLlm called for history summarization', () => {
    it('helperLlm called when history exceeds summarizeLimit', async () => {
        ...
        assert.ok(r.ok);
        assert.ok(helperLlm.callCount >= 1, 'helperLlm should be called for summarization');
    });
});
// ---------------------------------------------------------------------------
...
describe('helperLlm — summarization', () => {
    it('helperLlm NOT called when history is within limit', async () => {
        ...
        const r = await agent.process(messages);
        assert.ok(r.ok);
        assert.equal(helperLlm.callCount, 0, 'helperLlm should not be called for short history');
    });
...
describe('helperLlm — RAG translation', () => {
    it('helperLlm called for non-ASCII text translation', async () => {
        ...
        assert.ok(r.ok);
        assert.ok(helperLlm.callCount >= 1, 'helperLlm should be called for non-ASCII translation');
    });
    it('mainLlm used as fallback when no helperLlm for translation', async () => {
        ...
        const r = await agent.process('Покажи мне информацию о транзакции SE38');
        assert.ok(r.ok);
        assert.ok(llm.callCount >= 2, 'mainLlm should be called for both translation and chat');
    });
    it('translation skipped for ASCII text', async () => {
        ...
        assert.ok(r.ok);
        assert.equal(helperLlm.callCount, 0, 'helperLlm should not be called for ASCII text');
    });
});
// ---------------------------------------------------------------------------
...
describe('Regression — multiple action subprompts merged into one', () => {
    it('two action subprompts → assembler receives merged text containing both', async () => {
        ...
        const capturingAssembler = {
            async assemble(action, _retrieved, _history, _opts) {
        ...
        const agent = new SmartAgent(deps, DEFAULT_CONFIG);
        const r = await agent.process('Read table T100 and check its transport history');
        assert.ok(r.ok);
        assert.ok(capturedAction, 'assembler should have been called');
        assert.equal(capturedAction?.type, 'action');
        assert.ok(capturedAction?.text.includes('Read content of table T100'), 'merged text should contain first action');
        assert.ok(capturedAction?.text.includes('Check transport history of table T100'), 'merged text should contain second action');
...
describe('Regression — smartAgentEnabled=true behaves identically to undefined', () => {
    it('both return ok=true with same content', async () => {
        ...
        const r2 = await new SmartAgent(deps2, DEFAULT_CONFIG).process('do something');
        assert.ok(r1.ok);
        assert.ok(r2.ok);
...
describe('Regression — ragQueryK propagated through DefaultPipeline', () => {
    it('pipeline context uses agentConfig.ragQueryK when provided', async () => {
        ...
        assert.ok(toolsRag.queryCalls.length > 0, 'toolsRag should have been queried');
