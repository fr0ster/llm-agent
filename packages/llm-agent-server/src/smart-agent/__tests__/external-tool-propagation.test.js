...
function httpStreamRequest(port, body) {
    return new Promise((resolve, reject) => {
...
describe('External tool propagation — SmartAgent.process()', () => {
    it('returns stopReason=tool_calls and toolCalls when LLM requests external tool', async () => {
        ...
        assert.ok(r.value.toolCalls?.[0].function.arguments.includes('Berlin'), 'arguments should contain Berlin');
    });
...
describe('External tool propagation — SmartAgentServer HTTP streaming', () => {
    it('streams tool_calls deltas and finish_reason=tool_calls via SSE', async () => {
        ...
        try {
            ...
            const delta = toolChunk.choices[0]
                .delta;
            const toolCalls = delta.tool_calls;
            assert.equal(toolCalls.length, 1);
            assert.equal(toolCalls[0].function.name, 'get_weather');
            // Find finish_reason chunk
            const finishChunk = parsed.find((p) => {
                const choices = p.choices;
...
//# sourceMappingURL=external-tool-propagation.test.js.map