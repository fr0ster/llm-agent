...
function makeDelayedMcpClient(tools, results) {
    let callCount = 0;
    return {
        get callCount() {
            return callCount;
...
function makeToolCallingLlm(toolCallNames, finalContent) {
    let callCount = 0;
    return {
        get callCount() {
            return callCount;
...
async function collectStream(agent, message) {
    const heartbeats = [];
    let timing;
    const contentParts = [];
...
// Tests
// ---------------------------------------------------------------------------
const DEFAULT_CONFIG = { maxIterations: 5, mode: 'hard' };
describe('Heartbeat — emitted during slow MCP tool execution', () => {
    it('yields heartbeat chunks while tool is executing', async () => {
        const client = makeDelayedMcpClient([{ name: 'slow_tool', description: 'Slow', inputSchema: {} }], new Map([['slow_tool', { content: 'done', delayMs: 350 }]]));
    ...
    it('heartbeat elapsed increases over time', async () => {
        const client = makeDelayedMcpClient([{ name: 'slow_tool', description: 'Slow', inputSchema: {} }], new Map([['slow_tool', { content: 'done', delayMs: 450 }]]));
        ...
        for (let i = 1; i < heartbeats.length; i++) {
            assert.ok(heartbeats[i].elapsed > heartbeats[i - 1].elapsed, `heartbeat[${i}].elapsed (${heartbeats[i].elapsed}) should be > heartbeat[${i - 1}].elapsed (${heartbeats[i - 1].elapsed})`);
        }
    });
    it('no heartbeat for fast tools', async () => {
        const client = makeDelayedMcpClient([{ name: 'fast_tool', description: 'Fast', inputSchema: {} }], new Map([['fast_tool', { content: 'instant', delayMs: 0 }]]));
        ...
        const { heartbeats } = await collectStream(agent, 'test');
        assert.equal(heartbeats.length, 0, 'fast tool should not produce heartbeats');
    });
});
describe('Heartbeat — multiple concurrent tools', () => {
    it('heartbeats report only still-pending tools', async () => {
        ...
        assert.ok(!lateToolNames.has('fast_tool'), 'fast_tool should not appear in late heartbeats');
    });
});
describe('Timing — breakdown included in final chunk', () => {
    it('timing includes llm_call and tool entries', async () => {
        const client = makeDelayedMcpClient([{ name: 'my_tool', description: 'T', inputSchema: {} }], new Map([['my_tool', { content: 'result', delayMs: 50 }]]));
    ...
    it('timing durations are positive numbers', async () => {
        const client = makeDelayedMcpClient([{ name: 'tool', description: 'T', inputSchema: {} }], new Map([['tool', { content: 'ok', delayMs: 10 }]]));
        ...
        for (const entry of timing) {
            assert.ok(typeof entry.duration === 'number' && entry.duration >= 0, `${entry.phase} duration should be >= 0, got ${entry.duration}`);
        }
    });
    it('total >= sum of individual phases', async () => {
        const client = makeDelayedMcpClient([{ name: 'tool', description: 'T', inputSchema: {} }], new Map([['tool', { content: 'ok', delayMs: 50 }]]));
        const llm = makeToolCallingLlm(['tool'], 'done');
        const { deps } = makeDefaultDeps({ mcpClients: [client] });
        deps.mainLlm = llm;
        const agent = new SmartAgent(deps, DEFAULT_CONFIG);
        ...
        assert.ok(total.duration >= sumParts - 5, `total (${total.duration}ms) should be >= sum of parts (${sumParts}ms)`);
    });
});
