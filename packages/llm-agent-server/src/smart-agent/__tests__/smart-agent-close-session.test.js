...
import { InMemoryRag, InMemoryRagProvider, SimpleRagProviderRegistry, SimpleRagRegistry, } from '@mcp-abap-adt/llm-agent';
...
describe('SmartAgent.closeSession', () => {
    it('removes session-scoped collections with matching sessionId', async () => {
        ...
        assert.ok(res.ok, `createCollection failed: ${!res.ok && res.error.message}`);
        assert.ok(registry.get('session-A'), 'session-A should exist before closeSession');
        await agent.closeSession('S');
        assert.equal(registry.get('session-A'), undefined, 'session-A should be removed after closeSession');
    });
    it('preserves global/user collections', async () => {
        ...
        assert.ok(registry.get('global'), 'global collection should be preserved after closeSession');
    });
});
//# sourceMappingURL=smart-agent-close-session.test.js.map