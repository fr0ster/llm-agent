...
test('knowledgeSeed: parses entries (default artifactType=guidance) and defaults to empty', () => {
  ...
  const c = parseStepperCoordinatorConfig({
    ...
    knowledgeSeed: [
      ...
      {
        content: 'Read includes ONLY via GetInclude.',
  ...
  assert.match(c.knowledgeSeed[1].content, /GetInclude/);
});
