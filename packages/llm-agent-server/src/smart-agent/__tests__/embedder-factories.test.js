...
import { _resetPrefetchedForTests, prefetchEmbedderFactories, resolveEmbedder, } from '../embedder-factories.js';
afterEach(() => {
...
describe('factory registry — MissingProviderError', () => {
    it('resolveEmbedder throws MissingProviderError for unknown factory name', () => {
        assert.throws(() => resolveEmbedder('does-not-exist', {}), (err) => err instanceof MissingProviderError);
    });
    it('resolveEmbedder throws before prefetch', () => {
        assert.throws(() => resolveEmbedder('openai', {}), (err) => err instanceof MissingProviderError);
    });
...
//# sourceMappingURL=embedder-factories.test.js.map