...
describe('onBeforeStream hook', () => {
    it('transforms content when hook is set', async () => {
        ...
        const agent = new SmartAgent(deps, {
            ...DEFAULT_CONFIG,
            async *onBeforeStream(content, _ctx) {
        ...
        assert.ok(chunks.some((c) => c.includes('[transformed] raw response')), `Expected transformed content, got: ${JSON.stringify(chunks)}`);
    });
    it('streams content as-is when hook is not set', async () => {
        ...
        assert.ok(chunks.some((c) => c.includes('raw response')), `Expected raw content in stream, got: ${JSON.stringify(chunks)}`);
        assert.ok(!chunks.some((c) => c.includes('[transformed]')), 'Should not have transformed prefix');
    });
    it('hook receives messages in context', async () => {
        ...
        const agent = new SmartAgent(deps, {
            ...DEFAULT_CONFIG,
            async *onBeforeStream(content, ctx) {
                capturedCtx = ctx;
                yield content;
            },
        });
    ...
    it('hook can yield multiple chunks', async () => {
        ...
        const agent = new SmartAgent(deps, {
            ...DEFAULT_CONFIG,
            async *onBeforeStream(_content, _ctx) {
                yield 'chunk1';
                yield 'chunk2';
                yield 'chunk3';
            },
...
//# sourceMappingURL=on-before-stream.test.js.map