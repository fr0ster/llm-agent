...
export class FileSystemSkill {
    ...
    async getContent(args, _options) {
        try {
            ...
            return {
                ok: false,
                error: new SkillError(`Failed to get content for skill ${this.name}: ${err}`),
            };
        }
    }
    async listResources(_options) {
        try {
            ...
            return {
                ok: false,
                error: new SkillError(`Failed to list resources for skill ${this.name}: ${err}`),
            };
        }
    }
    async readResource(path, _options) {
        try {
            ...
            return {
                ok: false,
                error: new SkillError(`Failed to read resource ${path} for skill ${this.name}: ${err}`),
            };
        }
    }
}
