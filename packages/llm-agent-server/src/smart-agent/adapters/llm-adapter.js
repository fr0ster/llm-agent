...
function parseProviderResponse(raw, onDiagnostic) {
    ...
    if (providerRaw.usage) {
        usage = {
            promptTokens: providerRaw.usage.prompt_tokens ?? providerRaw.usage.input_tokens ?? 0,
            completionTokens: providerRaw.usage.completion_tokens ??
    ...
    if (Array.isArray(providerRaw.content)) {
        ...
        return {
            content: raw.content,
            toolCalls: toolCalls.length > 0 ? toolCalls : undefined,
            finishReason: providerRaw.stop_reason === 'tool_use' ? 'tool_calls' : 'stop',
            usage,
            raw: providerRaw,
        };
    }
    ...
    if (providerRaw.usage && !providerRaw.choices?.[0]?.delta) {
        return {
            ...
            usage: {
                ...
                totalTokens: providerRaw.usage.total_tokens ??
    ...
    async getModels(options) {
        ...
        try {
            ...
            const raw = options?.signal
                ? await withAbort(modelsPromise, options.signal, () => new LlmError('Aborted', 'ABORTED'))
                : await modelsPromise;
            ...
            if (options?.excludeEmbedding && this.provider.getEmbeddingModels) {
                ...
                const embeddingIds = new Set(embeddingModels.map((m) => (typeof m === 'string' ? m : m.id)));
    ...
    async chat(messages, tools, options) {
        try {
            const onDiagnostic = options?.sessionLogger
                ? (event) => options.sessionLogger?.logStep('llm_parse_diagnostic', event)
                : undefined;
            ...
            const agentOptions = options
                ...
                : undefined;
            const raw = await withAbort(this.agent.callWithTools(messages, mcpTools, agentOptions), options?.signal, () => new LlmError('Aborted', 'ABORTED'));
    ...
    async *streamChat(messages, tools, options) {
        try {
            ...
            const agentOptions = options
                ...
                : undefined;
            const stream = this.agent.streamWithTools(messages, mcpTools, agentOptions);
            ...
            if (err instanceof LlmError)
                yield { ok: false, error: err };
            else
                yield { ok: false, error: new LlmError(String(err)) };
        }
    }
