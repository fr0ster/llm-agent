...
export class LlmProviderBridge {
    ...
    async callWithTools(messages, tools, options) {
        ...
        const response = await this.provider.chat(messages, functions.length > 0 ? functions : undefined, options);
    ...
    async *streamWithTools(messages, tools, options) {
        ...
        for await (const chunk of this.provider.streamChat(messages, functions.length > 0 ? functions : undefined, options)) {
            // Yield text deltas
            if (chunk.content) {
            ...
            if (chunk.toolCalls) {
                for (const tc of chunk.toolCalls) {
                    ...
                    if (accumulated) {
                        ...
                        if (tc.name && !accumulated.name)
                            accumulated.name = tc.name;
                        if (tc.arguments)
                            accumulated.arguments += tc.arguments;
                    }
                }
            }
            // Track finish reason
