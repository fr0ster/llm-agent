...
describe('AnthropicApiAdapter.normalizeRequest', () => {
  ...
  it('parses a basic request', () => {
    ...
    assert.match(result.context.protocol.id as string, /^msg_/);
...
describe('AnthropicApiAdapter.transformStream', () => {
  ...
  it('emits correct event sequence for text content', async () => {
    ...
    const msgStart = parseData(events[0]) as Record<string, unknown>;
    ...
    const blockStart = parseData(events[1]) as Record<string, unknown>;
    assert.equal(blockStart.index, 0);
    assert.equal(
      (blockStart.content_block as Record<string, unknown>).type,
      'text',
    );

    // content_block_delta "Hello"
    assert.equal(events[2].event, 'content_block_delta');
    const delta1 = parseData(events[2]) as Record<string, unknown>;
    assert.equal((delta1.delta as Record<string, unknown>).type, 'text_delta');
    assert.equal((delta1.delta as Record<string, unknown>).text, 'Hello');

    // content_block_delta " world"
    assert.equal(events[3].event, 'content_block_delta');
    const delta2 = parseData(events[3]) as Record<string, unknown>;
    assert.equal((delta2.delta as Record<string, unknown>).text, ' world');
    ...
    const msgDelta = parseData(events[5]) as Record<string, unknown>;
    assert.equal(
      (msgDelta.delta as Record<string, unknown>).stop_reason,
      'end_turn',
    );
  ...
  it('skips heartbeat-only chunks', async () => {
    ...
    for (const e of events) {
      const data = parseData(e) as Record<string, unknown>;
  ...
  it('emits tool_use blocks correctly', async () => {
    ...
    assert.equal(events[1].event, 'content_block_start');
    const textBlock = parseData(events[1]) as Record<string, unknown>;
    assert.equal(
      (textBlock.content_block as Record<string, unknown>).type,
      'text',
    );
    assert.equal(events[2].event, 'content_block_delta');
    ...
    const toolBlock = parseData(events[4]) as Record<string, unknown>;
    assert.equal(
      (toolBlock.content_block as Record<string, unknown>).type,
      'tool_use',
    );
    assert.equal(
      (toolBlock.content_block as Record<string, unknown>).id,
      'toolu_1',
    );
    assert.equal(
      (toolBlock.content_block as Record<string, unknown>).name,
      'search',
    );

    assert.equal(events[5].event, 'content_block_delta');
    const inputDelta = parseData(events[5]) as Record<string, unknown>;
    assert.equal(
      (inputDelta.delta as Record<string, unknown>).type,
      'input_json_delta',
    );
    assert.equal(
      (inputDelta.delta as Record<string, unknown>).partial_json,
    ...
    const msgDelta = parseData(events[7]) as Record<string, unknown>;
    assert.equal(
      (msgDelta.delta as Record<string, unknown>).stop_reason,
      'tool_use',
    );
  ...
  it('emits closing events when stream ends without finishReason', async () => {
    ...
    assert.equal(events.length, 6);
    assert.equal(events[4].event, 'message_delta');
    const msgDelta = parseData(events[4]) as Record<string, unknown>;
    assert.equal(
      (msgDelta.delta as Record<string, unknown>).stop_reason,
      'end_turn',
    );
  ...
  it('includes usage in message_delta', async () => {
    ...
    const msgDelta = parseData(msgDeltaEvent) as Record<string, unknown>;
    const usage = msgDelta.usage as Record<string, number>;
    assert.equal(usage.input_tokens, 10);
    assert.equal(usage.output_tokens, 5);
  });
