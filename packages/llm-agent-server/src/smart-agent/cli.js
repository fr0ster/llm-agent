...
import { generateConfigTemplate, loadYamlConfig, resolveSmartServerConfig, } from './config.js';
...
if (!configArg && !fs.existsSync(DEFAULT_CONFIG_FILE)) {
    ...
    process.stderr.write(`No config file found. Created ${DEFAULT_CONFIG_FILE} with defaults.\n` +
...
let baseConfig;
try {
    baseConfig = resolveSmartServerConfig(args, yaml, process.env);
}
catch (err) {
...
// ---------------------------------------------------------------------------
{
    ...
    const embedderNames = new Set();
    const pushEmbedderFor = (cfg) => {
        if (cfg.type && cfg.type !== 'in-memory') {
            embedderNames.add(cfg.embedder ?? (cfg.type === 'openai' ? 'openai' : 'ollama'));
        }
...
{
    const ragCfg = baseConfig.rag;
    const ragBackendNames = new Set();
    const peerBackend = (t) => t === 'qdrant' || t === 'hana-vector' || t === 'pg-vector';
    if (ragCfg && peerBackend(ragCfg.type))
        ragBackendNames.add(ragCfg.type);
    const pipelineRag = baseConfig.pipeline?.rag;
    if (pipelineRag) {
        for (const cfg of Object.values(pipelineRag)) {
            if (cfg?.type && peerBackend(cfg.type))
                ragBackendNames.add(cfg.type);
        }
    }
