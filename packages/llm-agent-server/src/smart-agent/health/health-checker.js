export class HealthChecker {
    ...
    async check() {
        ...
        const cbStatuses = this.circuitBreakers.length > 0
        ...
        const mcpAllOk = components.mcp.length === 0 || components.mcp.every((m) => m.ok);
        const anyCircuitOpen = this.circuitBreakers.some((cb) => cb.state === 'open');
        let status;
        if (!llmOk) {
            status = 'unhealthy';
        }
