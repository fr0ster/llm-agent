...
export class ToolSelectHandler {
    async execute(ctx, config, span) {
        ...
        if (allRagResults.length === 0 && ctx.mcpTools.length > 0) {
            const k = config.k ?? ctx.config.ragQueryK ?? 20;
            const queryTextSource = typeof config.queryText === 'string'
                ? config.queryText === 'toolQueryText'
                    ? (ctx.toolQueryText ?? ctx.ragText)
                    : config.queryText
                : undefined;
            const queryText = queryTextSource ?? ctx.toolQueryText ?? ctx.ragText ?? ctx.inputText;
            ...
            const queryResults = await Promise.all(storeEntries.map(async ([name, store]) => ({
                name,
        ...
        const filtered = ctx.toolAvailabilityRegistry.filterTools(ctx.sessionId, ctx.selectedTools);
...
//# sourceMappingURL=tool-select.js.map