 ...
 */
import { NoopQueryExpander, NoopToolCache, StreamingLlmCallStrategy, } from '@mcp-abap-adt/llm-agent';
...
export class DefaultPipeline {
    ...
    async execute(input, history, options, yieldChunk, externalTools) {
        const rootSpan = this.resolvedTracer.startSpan('pipeline.execute');
        const ctx = this._buildContext(input, history, options, yieldChunk, externalTools);
        try {
    ...
    _buildStages() {
        ...
        stages.push({ id: 'tool-select', type: 'tool-select' }, { id: 'assemble', type: 'assemble' }, { id: 'tool-loop', type: 'tool-loop' }, { id: 'history-upsert', type: 'history-upsert' });
        return stages;
    }
    /**
     ...
     */
    _buildContext(input, history, options, yieldChunk, externalTools) {
