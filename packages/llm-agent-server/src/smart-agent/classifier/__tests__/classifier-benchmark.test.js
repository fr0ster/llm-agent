...
const GOLDEN_CORPUS = [
    ...
    {
        input: 'The previous result was wrong. Now get the source of ZCL_FIXED and explain what ABAP packages are.',
        expected: [
    ...
    {
        input: 'Search for ZCL_HELPER, get its source, check it with ATC, and create a transport',
        expected: [
...
function makeStubLlm(responses) {
    return {
        async chat(messages) {
...
function countAccuracy(results) {
    if (results.length === 0)
        return 1;
    let correct = 0;
    for (const { actual, expected } of results) {
...
function perTypePrecisionRecall(results, type) {
    let truePositives = 0;
...
function multiIntentAccuracy(results) {
    if (results.length === 0)
        return 1;
    let correct = 0;
    for (const { actual, expected } of results) {
...
describe('Classifier Benchmark', () => {
    it('type accuracy >= 90% across golden corpus', async () => {
        ...
        assert.ok(accuracy >= 0.9, `Type accuracy = ${(accuracy * 100).toFixed(1)}%, expected >= 90%`);
    });
    it('count accuracy >= 85% across golden corpus', async () => {
        const results = await runBenchmark();
        const accuracy = countAccuracy(results);
        assert.ok(accuracy >= 0.85, `Count accuracy = ${(accuracy * 100).toFixed(1)}%, expected >= 85%`);
    });
    it('per-type precision >= 80% for each SubpromptType', async () => {
        ...
        for (const type of types) {
            ...
            assert.ok(precision >= 0.8, `Precision for "${type}" = ${(precision * 100).toFixed(1)}%, expected >= 80%`);
        }
    });
    it('per-type recall >= 80% for each SubpromptType', async () => {
        ...
        for (const type of types) {
            ...
            assert.ok(recall >= 0.8, `Recall for "${type}" = ${(recall * 100).toFixed(1)}%, expected >= 80%`);
        }
    });
    it('multi-intent entries fully decomposed >= 80%', async () => {
        const results = await runBenchmark();
        const accuracy = multiIntentAccuracy(results);
        assert.ok(accuracy >= 0.8, `Multi-intent accuracy = ${(accuracy * 100).toFixed(1)}%, expected >= 80%`);
    });
    ...
    it('all single-intent entries produce exactly 1 subprompt', async () => {
        ...
        for (const { entry, result } of singleEntries) {
            assert.equal(result.actual.length, 1, `Single-intent "${entry.input}" produced ${result.actual.length} subprompts, expected 1`);
        }
    });
});
//# sourceMappingURL=classifier-benchmark.test.js.map