...
describe('LlmClassifier — intent types', () => {
    it('action intent', async () => {
        const llm = makeLlm('[{"type":"action","text":"What is the capital of France?"}]');
    ...
    it('feedback intent', async () => {
        const llm = makeLlm('[{"type":"feedback","text":"Your last answer was wrong."}]');
        const classifier = new LlmClassifier(llm);
    ...
    it('multi-intent — 2 subprompts of different types', async () => {
        const llm = makeLlm('[{"type":"fact","text":"Earth is round."},{"type":"action","text":"Tell me about gravity."}]');
        const classifier = new LlmClassifier(llm);
