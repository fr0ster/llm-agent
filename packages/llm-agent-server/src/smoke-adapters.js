...
async function main() {
    ...
    console.log(`   LLM:  DeepSeek (${process.env.DEEPSEEK_MODEL || 'deepseek-chat'})`);
    ...
    const llmAdapter = new LlmAdapter(new LlmProviderBridge(llmProvider), {
        ...
        getEmbeddingModels: () => llmProvider.getEmbeddingModels?.() ?? Promise.resolve([]),
    });
    ...
    const chatToolsResult = await llmAdapter.chat(toolMessages, toolsResult.value);
    if (!chatToolsResult.ok) {
        fail('chat with tools', chatToolsResult.error.message);
    }
    else {
        const tc = chatToolsResult.value.toolCalls ?? [];
        ok('chat with tools', `finishReason=${chatToolsResult.value.finishReason}, toolCalls=${tc.length}`);
        for (const c of tc) {
            console.log(`     - ${c.name}(${JSON.stringify(c.arguments)})`);
        }
        if (chatToolsResult.value.content) {
            console.log(`     content: ${chatToolsResult.value.content.slice(0, 80)}…`);
        }
    }
    console.log();
    // -------------------------------------------------------------------------
    ...
    controller.abort();
    const abortResult = await llmAdapter.chat([{ role: 'user', content: 'This should never reach DeepSeek.' }], undefined, { signal: controller.signal });
    if (!abortResult.ok && abortResult.error.code === 'ABORTED') {
        ok('AbortSignal rejected with ABORTED');
    }
    else {
        fail('AbortSignal', `expected ABORTED error, got: ${JSON.stringify(abortResult)}`);
    }
    console.log();
    // -------------------------------------------------------------------------
    // Summary
...
//# sourceMappingURL=smoke-adapters.js.map