...
describe('OpenAIProvider — constructor', () => {
    it('throws when apiKey is missing', () => {
        assert.throws(() => new OpenAIProvider({ apiKey: '' }), /API key is required/);
    });
...
describe('OpenAIProvider — streamChat() usage', () => {
    ...
    it('forwards tool_calls deltas in normalized form (regression: #119)', async () => {
        ...
        provider.client.post = async () => ({
            data: (async function* () {
                yield Buffer.from('data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_1","function":{"name":"get_weather","arguments":""}}]},"finish_reason":null}]}\n\n');
                yield Buffer.from('data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\\"city\\":"}}]},"finish_reason":null}]}\n\n');
                yield Buffer.from('data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\\"Kyiv\\"}"}}]},"finish_reason":null}]}\n\n');
                yield Buffer.from('data: {"choices":[{"delta":{},"finish_reason":"tool_calls"}]}\n\n');
                yield Buffer.from('data: [DONE]\n\n');
            })(),
        });
        const chunks = [];
    ...
    it('yields usage-only chunk at end of stream', async () => {
        ...
        provider.client.post = async () => ({
            data: (async function* () {
                yield Buffer.from('data: {"choices":[{"delta":{"content":"Hello"},"finish_reason":null}]}\n\n');
                yield Buffer.from('data: {"choices":[{"delta":{},"finish_reason":"stop"}]}\n\n');
                yield Buffer.from('data: {"choices":[],"usage":{"prompt_tokens":5,"completion_tokens":1,"total_tokens":6}}\n\n');
                yield Buffer.from('data: [DONE]\n\n');
            })(),
        });
        const chunks = [];
