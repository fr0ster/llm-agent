...
export class HanaVectorRagProvider extends AbstractRagProvider {
    ...
    constructor(cfg) {
        ...
        if (cfg.idStrategyFactory)
            this.idStrategyFactory = cfg.idStrategyFactory;
    }
    ...
    async listCollections() {
        try {
            const client = this.requireClient();
            const rows = await client.query(this.connection.schema
                ? 'SELECT TABLE_NAME FROM SYS.TABLES WHERE SCHEMA_NAME = ?'
                : 'SELECT TABLE_NAME FROM SYS.TABLES WHERE SCHEMA_NAME = CURRENT_SCHEMA', this.connection.schema ? [this.connection.schema] : []);
    ...
    requireClient() {
        if (!this.clientFactory) {
            throw new Error('HanaVectorRagProvider deleteCollection/listCollections require clientFactory; provide one to enable schema-level operations');
        }
