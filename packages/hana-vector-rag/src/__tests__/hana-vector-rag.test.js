...
describe('HanaVectorRag', () => {
    ...
    it('query returns results mapped from rows', async () => {
        ...
        const rag = new HanaVectorRag({ collectionName: 'docs', dimension: 3, embedder: makeEmbedder(3) }, client);
        const res = await rag.query({ text: 'test query', toVector: async () => [0.1, 0.2, 0.3] }, 5);
    ...
    it('rejects invalid collection name at construction', () => {
        assert.throws(() => new HanaVectorRag({ collectionName: "bad'; DROP", embedder: makeEmbedder() }, makeFakeClient()), (err) => err.code === 'INVALID_COLLECTION_NAME');
    });
});
//# sourceMappingURL=hana-vector-rag.test.js.map