...
describe('McpClientAdapter.listTools()', () => {
    it('success — maps raw tools to McpTool[]', async () => {
        const adapter = new McpClientAdapter(makeClient({ listTools: async () => TOOLS_FIXTURE }));
    ...
    it('error — wraps in McpError', async () => {
        const adapter = new McpClientAdapter(makeClient({
            listTools: async () => {
    ...
    it('pre-aborted signal → ABORTED', async () => {
        const adapter = new McpClientAdapter(makeClient({ listTools: async () => [] }));
