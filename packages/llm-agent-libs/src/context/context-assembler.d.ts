import type { HistoryEntry, IContextAssembler, Message } from '@mcp-abap-adt/llm-agent';
import { AssemblerError, type CallOptions, type McpTool, type RagResult, type Result, type Subprompt } from '@mcp-abap-adt/llm-agent';
...
export declare class ContextAssembler implements IContextAssembler {
    ...
    assemble(action: Subprompt, retrieved: {
        ragResults: Record<string, RagResult[]>;
        tools: McpTool[];
        recentActions?: string[];
    }, history: HistoryEntry[], options?: CallOptions): Promise<Result<Message[], AssemblerError>>;
}
