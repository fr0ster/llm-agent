...
function buildSystemContent(ragResults, provenance, sectionHeaders, recentActions) {
    const sections = [];
    for (const [key, results] of Object.entries(ragResults)) {
        const header = sectionHeaders[key] ?? titleCase(key);
        const section = buildSection(header, results.map((r) => formatRagEntry(r, provenance)));
        if (section)
            sections.push(section);
    }
    if (recentActions && recentActions.length > 0) {
        const section = buildSection('Recent Actions', recentActions.map((a) => `- ${a}`));
        if (section)
            sections.push(section);
    }
 ...
 */
function applyTokenBudget(ragResults, actionTokens, maxTokens, provenance, sectionHeaders, recentActions) {
    ...
    const totalTokens = () => {
        const content = buildSystemContent(mutableResults, provenance, sectionHeaders, recentActions);
    ...
    async assemble(action, retrieved, history, options) {
        try {
            ...
            if (this.maxTokens !== undefined) {
                finalResults = applyTokenBudget(sortedResults, estimateTokens(action.text), this.maxTokens, this.includeProvenance, this.sectionHeaders, retrieved.recentActions);
            }
            const systemContent = buildSystemContent(finalResults, this.includeProvenance, this.sectionHeaders, retrieved.recentActions);
            const messages = [];
            const preamble = this.systemPromptPreamble ?? '';
            const toolGuidance = 'Use available tools when they can accomplish the task. When an action is impossible with available tools — say so clearly and do not attempt it.';
            ...
            const windowedMessages = this.historyRecencyWindow !== undefined &&
