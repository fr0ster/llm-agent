...
export class SkillSelectHandler {
    async execute(ctx, _config, span) {
        ...
        if (ragSkillNames.size === 0) {
            const k = _config.k ?? ctx.config.ragQueryK ?? 15;
            ...
            const queryResults = await Promise.all(storeEntries.map(([, store]) => store.query(embedding, k, ctx.options)));
        ...
        for (const skill of ctx.selectedSkills) {
            const contentResult = await skill.getContent(ctx.skillArgs || undefined, ctx.options);
...
//# sourceMappingURL=skill-select.js.map