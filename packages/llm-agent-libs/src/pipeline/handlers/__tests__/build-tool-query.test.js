...
function makeRagResult(id, text, score = 0.5) {
    return { text, metadata: { id }, score };
}
...
describe('BuildToolQueryHandler', () => {
    it('composes ragText + snippets + skills and writes toolQueryText', async () => {
        ...
        assert.match(ctx.toolQueryText ?? '', /abap-basics: SE16 \/ SE11 quick reference/);
    });
    it('excludes tool:* RAG entries from the snippet block', async () => {
        const ctx = makeCtx({
            ...
            ragResults: {
                mixed: [
                    makeRagResult('tool:GetTable', 'Tool description that must be skipped'),
    ...
    it('truncates to maxChars with trailing ellipsis', async () => {
        ...
        await new BuildToolQueryHandler().execute(ctx, { maxChars: 100 }, makeSpan());
    ...
    it('skips sections when flags are disabled', async () => {
        ...
        await new BuildToolQueryHandler().execute(ctx, { includeRagSnippets: false, includeSkills: false }, makeSpan());
    ...
    it('respects skipStores', async () => {
        ...
        await new BuildToolQueryHandler().execute(ctx, { skipStores: ['history'] }, makeSpan());
        assert.match(ctx.toolQueryText ?? '', /keep-me/);
        assert.doesNotMatch(ctx.toolQueryText ?? '', /skip-me/);
    });
});
//# sourceMappingURL=build-tool-query.test.js.map