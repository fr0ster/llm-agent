...
DEPENDENCIES ARE DATAFLOW — set "dependsOn" whenever a step USES the OUTPUT of earlier steps. Each step runs in a SEPARATE worker that does NOT share your context; it receives a predecessor's output ONLY if that predecessor is in its "dependsOn". So a step that analyses/reviews/synthesises/uses data gathered by earlier steps MUST list every such step in "dependsOn" — otherwise it runs in PARALLEL with them and WITHOUT their data, and will redundantly re-fetch. Prose like "using the source from steps 1-3" is NOT enough; encode it as "dependsOn":["step1","step2","step3"]. Steps with NO dependsOn run concurrently — use that only for genuinely independent steps.
GATHER WITHOUT OVERLAP — never emit two steps that fetch the SAME data. A DISCOVERY step (enumerate/list what exists, or read a shell to find references) must NOT also read the full bodies — emit ONE separate read step that "dependsOn" the discovery and reads the bodies. Parallel steps (no dependsOn between them) MUST fetch DISJOINT data; if two steps would read overlapping artefacts, either merge them into one step or chain them with "dependsOn" so the later one reuses the earlier one's output instead of re-fetching. One read per artefact across the whole plan.
