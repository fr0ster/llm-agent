...
- **Stepper coordinator.** Coordinator/worker is a `Stepper` (planner +
  reviewer + interpreter + executor) over a per-session knowledge-RAG
  blackboard, opt-in via `coordinator.mode`. Legacy DAG configs (no
  `coordinator.mode`) are unaffected.

- **Two execution modes** (`coordinator.mode`):
  - `cyclic-react` — single executor loop, no planning overhead. Best for bounded tasks.
  - `planned-react` _(default)_ — LLM planner decomposes the request; Stepper
    workers execute steps and accumulate findings in the knowledge-RAG blackboard.

  > The recursive `deep-stepper` mode (each Stepper spawns child Steppers up to
  > `maxDepth`) is **NOT shipped in 18.0** — its root planner needs hardening
  > before production. It is under development on the
  > `feature/recursive-deep-stepper` branch, targeted for a later release.
...
- **Stepper examples.** `docs/examples/stepper/` adds two production-shaped
  configs (`01-cyclic-react.yaml`, `02-planned-react.yaml`), a shared
  `worker.yaml` subagent pipeline, and a `README.md` covering both modes,
  `/v1/sessions` resume flow, and the readOnly tool-safety policy.
...
Pre-release hardening from live SAP validation of the two shipped modes:
...
### Breaking changes
- **`ISkillMeta.allowedTools` removed** along with the `allowed-tools` frontmatter mapping in `ClaudeSkillManager`. The field was never enforced at runtime, was incorrectly borrowed from Claude Code's subagent format (subagents are a different abstraction from skills), and the project's real skills never used it. Custom `ISkill` implementations that declared `allowedTools` will get a TypeScript compile error on upgrade — delete the declaration; runtime behavior is unchanged (the field was never read by any handler).

  Skill authors who want to recommend specific MCP tools should mention them in the SKILL.md body — the LLM reads the prose, sees the recommendation, and selects from the full MCP catalog. Tool authorization (read-only, sandbox, per-tenant restrictions) is a separate MCP-transport-layer concern, not a skill concern.

### Documentation
- Removed `allowed-tools` from skill examples in `docs/EXAMPLES.md` and `docs/ARCHITECTURE.md` and from the commented sample in `docs/examples/11-skills.yaml`. Removed the field reference row from the EXAMPLES.md frontmatter-fields table.
...
### Breaking
- **Repository is now an npm-workspaces monorepo** with two initial packages. The single `@mcp-abap-adt/llm-agent` package is split:
  - `@mcp-abap-adt/llm-agent` — interfaces, types, and lightweight default RAG implementations.
  - `@mcp-abap-adt/llm-agent-server` — default `SmartAgent`, pipeline, LLM providers, MCP client, HTTP server, and the three bin CLIs.
- **Imports** — consumers must switch to one or both v10 packages. `SmartAgent`, `SmartAgentBuilder`, `DefaultPipeline`, pipeline handlers, LLM providers, `FallbackRag`, `MCPClientWrapper`, agent hierarchy, resilience wrappers, skill managers now live in `@mcp-abap-adt/llm-agent-server`. Interfaces, `InMemoryRag` / `VectorRag` / `QdrantRag`, RAG providers, registries, strategies, corrections, and errors live in `@mcp-abap-adt/llm-agent`. See `docs/MIGRATION-v10.md` for the full symbol-by-symbol table.
- **CLI** — `npx @mcp-abap-adt/llm-agent` is no longer valid; use `npx @mcp-abap-adt/llm-agent-server`. All three bins (`llm-agent`, `llm-agent-check`, `claude-via-agent`) preserved and now shipped by `@mcp-abap-adt/llm-agent-server`.
- **Deep sub-exports** — `./testing`, `./smart-server`, `./otel` sub-paths moved from the old package to `@mcp-abap-adt/llm-agent-server`.

### Added
- **Workspace tooling** — root `tsconfig.base.json` shared by both packages; TypeScript project references ensure core builds before server. `@changesets/cli` installed with `fixed: [[core, server]]` lock-step versioning.
- **Per-package CHANGELOGs** — `packages/llm-agent/CHANGELOG.md` and `packages/llm-agent-server/CHANGELOG.md` generated by Changesets; this root CHANGELOG remains the aggregated history.
- **Migration doc** — `docs/MIGRATION-v10.md` with install / import / CLI migration tables.
- **Monorepo README** — repo root `README.md` rewritten as a package index.

### Changed
- **CI workflows** — `.github/workflows/ci.yml` runs generic root scripts (`lint:check` / `build` / `test`) that fan out via workspaces. `.github/workflows/release.yml` trigger on `v*` tags preserved; npm publish remains manual.
- **Test suites move with their code** — 224 tests live in core, 551 in server. Total 775 pass.
- **Core package dependencies** — `@mcp-abap-adt/llm-agent` carries `axios` (for `QdrantRag`) and `@sap-ai-sdk/orchestration` (for `SapAiCoreEmbedder`) in v10.0. These will be extracted into dedicated provider packages in follow-up 10.1.0+ releases, at which point core's runtime dependencies shrink to the minimum needed for the interfaces.

### Closes
- Lays foundation for incremental provider extraction planned in 10.1.0+: `@mcp-abap-adt/openai-provider`, `@mcp-abap-adt/anthropic-provider`, `@mcp-abap-adt/sap-aicore-provider`, `@mcp-abap-adt/ollama-provider`, `@mcp-abap-adt/hana-vector-provider`.
